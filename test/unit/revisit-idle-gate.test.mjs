...
test('⛔ a missing registry is not an idle Sotera', () => {
  const v = checkIdleGate({ config: { memory: {} } }, null)
  assert.equal(v.run, false)
  assert.equal(v.reason, GATE.configUnreadable, 'cannot tell ⇒ closed branch, same as an unreadable config')
})
...
test('⭐ the route exposes ONE registry to the background lane, never a second one', () => {
  const route = strip(read('../../Backend/app/routes/v1/chat-site.route.js'))
  assert.match(route, /fastify\.decorate\('steerReg', steerReg\)/)
  assert.equal((route.match(/createSteerRegistry\(\)/g) ?? []).length, 1,
    '⛔ exactly one registry instance — two would drift about whether a turn is running')
