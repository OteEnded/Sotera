...
import { MECHANISM, mechanismOf, PRESENCE, derivedFromOf, lineageRestatesTheOccasion } from '../../Backend/app/components/memory-lineage.js'
...
if (dev) {
  ...
  if (msg && other) {
    const lineage = { derivedFrom: { basis: PRESENCE.inContext, memoryIds: [other.id], via: 'check' } }
    const row = await one(
