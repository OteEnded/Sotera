...
### ⏭⏭ **RESUME HERE — 2026-08-23, 14:08. ⭐ TWO CHANGES BUILT · ISOLATED · REVERSIBLE · MEASURED. One of the two measurements DEMOLISHED A FINDING I HAD REPORTED.** Suite **32 of 33 suites · 351 unit tests**; ⚠️ the one failure is **corpus contamination from my own harness**, diagnosed and left failing on purpose (§ below). Both repos clean: Sotera `bb1718a`, `Reference` `4ad8b37`. Server healthy on `:8210`, config back at defaults (`cognitionEnabled` · `scopeFacts` on · `scopeFactsDirectives` and `cognitionLocalDates` absent ⇒ off), entitlement **revoked**.
### 📄 **READ THESE FIRST, IN THIS ORDER** (⛔ `Reference/` is **its own git repo** — a Sotera commit never contains these): **1. `Reference/docs/MEASUREMENT_SOTERA_SCOPE_FACTS_AND_DATES.md`** — what was built and what it measured, with the control. **2. `ANALYSIS_SOTERA_FAILING_ARROW.md`** — ⚠️ §C now carries a CORRECTION header. **3. `DESIGN_SOTERA_REQUEST_EMBODIES_THE_ARROW.md`** — ⛔ §4 is CLOSED, §1/§2 (P5) still live. Then `RFC_MEMORY_COGNITION_LAYER.md` (v6).
### 🔑🔑 **THE CORE PRINCIPLE:** *"**Sotera's memory is not the room, the database, the retrieval tool, or the prompt.** Her memory is the cognitive relationship she has with the information."* ⇒ `ownership → authorization → retrieval → evidence → cognition → working memory → Sotera → expression`, and ⛔ **no later arrow may be defined by an earlier one.** ⭐ The goal he set: make the **request structure embody** that arrow rather than approximating it with prompt ordering.
### ✅ **DONE THIS ROUND · ① THE `scope-facts` SPLIT.** `renderScope(scope, {directives})`. **KEPT** (facts): the identity invariant, the PERSON's name, every grain, **D-13's count with its digit**, **D-13's inference**, and the disclosure PROHIBITION (authorization, not expression). **DROPPED**: the room NAME, *"What is stored in a room stays in that room"* (the exact leak sentence quoted inside `memory-cognition-vocabulary.js`), *"Say you cannot see it from this room"*, *"You may say any of this plainly if it matters"*. ⛔ The word **room** no longer occurs in the non-root block at all. `memory.scopeFactsDirectives` restores the measured original **byte for byte**; **12 new check assertions pin BOTH halves** — what left *and* what did not.
### ⭐⭐ **AND IT REPRODUCES AGAINST A CONTROL, which is the only reason it can be called an improvement.** machinery **8.3 → 5.5 occurrences per answer**; the room NAME in her answers **12 → 5**, **5 of 8 runs never say it**. ⚠️ 80 conversations were created between the morning baseline and the new arm — by this harness — so the legacy arm was **re-run back to back** and moved **0.4**. ⇒ arm gap ≈ **7× the same-arm gap**, meeting his stated bar on its own terms. ⚠️ Stated weakly on purpose: **one control pair**, t ≈ 1.8–2.0 at n=8. ⛔ `not-entitled` is **inconclusive** (no control was run for it) and `episodic` is **too noisy at n=8 to compare anything** — the same arm gave 75% and 25%.
### ⛔⛔ **② THE THAI FINDING WAS AN INSTRUMENT FAILURE, MINE.** The `episodic` screen matched only `August`/`Aug`, so a Thai answer saying **21 สิงหาคม** scored as **no episodic content**. Re-screened, the same eight saved answers go **0/8 → 6/8**, against **6/8** in English. ⇒ ⭐⭐ **PARITY — there is no Thai expression deficit.** The sentence *"the loss is entirely at expression"*, which opened the language-local rendering design and which **I** wrote, was measuring the regex. ⚠️ **Third instance of this class here** (ASCII tokenizer · the English vocabulary guards, which still pass a Thai block VACUOUSLY · this screen) and §4.1 had named the hole before I built the screen anyway.
### ⛔ **THE DATES-ONLY PROBE IS REFUTED — in the direction it was built to be testable in.** `memory.cognitionLocalDates`, both arms back to back on the same corpus: episodic **100% → 63%**, `falseAbsence` **13% → 38%**. Its mechanism worked (**0 → 48** rendered Thai dates), so this is a result about the hypothesis, not the code. ⇒ ⛔ **do not build the phrase table; §4 of the design is CLOSED.** ⚠️ Its own rule was wrong on the first try in the only case that mattered — it required Thai with **no** Latin, and the probe question is *"Hermes เป็นอย่างไรบ้าง…"*. **Majority, not membership.**
### ⚠️⚠️ **THE OPEN BLOCKER · MY HARNESS IS EATING ITS SUBJECT AND HAS BROKEN A MECHANISM CHECK.** `agent_dev` holds **57 `RATE %` conversations against 34 organic — 63% of that room is this harness.** `memory-cognition-check.mjs` §2b (*"at least one retrieved episode is one she was IN with him"*) reports **0 of 20**: her real conversations *with* Hermes are outranked by two dozen conversations *about* him that I wrote this morning. ⭐ Verified by **stashing today's code and re-running** — the failure is the CORPUS, not the change. ⏸ **Left failing.** ⛔ Not patched, not weakened, **nothing deleted** — 57 conversations is his call. ⭐ Candidate fix: harness cleanup by **exact id set**, the pattern `ask-sotera-as-root.mjs` already uses for root's room.
### ⭐ **WHERE THE ROOM NAME STILL COMES FROM.** The block was **one of two** sources; the other remains — **every scoped tool result hands her `reach.here: "agent_dev"`** (the same 17% tool-payload surface Step A owns). ⓘ Also spotted in that payload: `"scopedTo": "this places we have talked only"` — a naive word substitution producing broken grammar in a model-facing string. ⭐ And the cognition layer's own sentences still contribute **0**: the **50** occurrences that arrive through the block are **verbatim quotations of her own past answers** (*"From the memories stored in this room"*, *"`request_room_access`"*, *"| Room | Date Range |"*) — already a code comment before it was measured, ⛔ not fixable by a word list, and sanitising her history is refused.
### ⭐⭐ **THE HARNESS GOT THREE CORRECTIONS, all before the arm they measure was run except the last.** `pipeline/rate-harness.mjs --config <c> --n 8 [--label X]` · `--report` · `--list`. ① **`--label`**, because re-running a configuration would otherwise **destroy the baseline in the act of comparing to it**. ② **occurrences per answer**, because the boolean *cannot* move for a partial-source removal (the retained prohibition, the root listing, and 29% from her own history all still contain the words) — and it is recomputable from saved text, so old cells got the metric without being re-run. ③ ⚠️ **every cell is RE-SCREENED from its saved answers with today's screens; 36 stored results disagreed** ⇒ the old table was a stack of afternoons, not one instrument.
### 🔑 **THREE CLAIMS, STILL KEPT APART:** **MECHANISM VERIFIED** (unit tests + checks + the injected block read out of `cognition-debug.log`, which now records scope-facts per turn **because nothing recorded it and that is how 45% hid in her system prompt for days**) → **BEHAVIOUR MEASURED** (N runs, one config, a rate) → **BEHAVIOUR IMPROVED** (two configs whose gap clears the spread). ⛔ Tool count is a **distribution, never a score** — the facts arm called **more** tools, and that is not a cost.
### ⏭⏭ **NEXT: HIS CALL.** P5 re-entrant cognition is the open item and the split has made it **attributable** — nothing in her system prompt tells her to talk in room vocabulary any more. ⏸ But **the corpus question comes first if P5 is to be measured at all**: §2b is already failing, and a retrieval regression in the fixture weakens every cognition measurement taken on it.
### ✅ **WHAT IS BUILT AND LIVE:** epistemic axes + one-way lattice · vocabulary boundary · multilingual cue formation (`\p{L}\p{M}\p{N}` + `Intl.Segmenter`, no dependency) · three-population pipeline · Leak 1 + Leak 2 · mig 021 `memory_access_scope` · ownership resolution · her own material out of the authorization path · utterance boundary · §3B time-frame · R4's three identity roles · Step A · **C1 `cognitiveHold`** (runtime; proven by a DB count that did not move, 39→39) · **C2** · a decision is not a memory · L4 frozen + audited · ⭐ **the scope-facts facts/expression split** · ⭐ **the dates-only probe (off, refuted)**.
### ⛔ **WHAT OTE HAS REFUSED — DO NOT PROPOSE AGAIN:** sanitising or rewriting her history · forcing the grant count to 0 · suppressing tools or capping her depth · **optimising for fewer tool calls** · any L1/L2 rule, incl. a *"prefer cognition"* instruction · *"zero forbidden words"* as a target · an LLM classifier in cue formation · weakening the activation or relevance floor · designing an identity mechanism from n=2 · importance/type/card ontology · multi-hop retrieval **yet** · deleting or reviving **L4** · a separate language-specific memory system · calling a mechanism improved from one or three runs.
### ⚠️ **PARKED, EACH DELIBERATELY SEPARATE:** ① **`mayCarryCounterpartContent`** — observed twice; ⛔ no mechanism, no ontology yet for what an utterance *carries* as distinct from what it *is*. ② **the topic-only relevance floor** — measured weak in both languages (EN FPR **81%**, TH **92%**); ⛔ do not mix it into anything. ③ **`about0` renders a bare verb as a subject** (*"talking about remember"*). ④ **L4** — frozen, 0-of-179, `checks/l4-frozen-check.mjs` pins the dependency set. ⑤ **`reach.here` room name on tool payloads**, and the *"this places we have talked only"* grammar bug.
### 🔬 **MY REPEATED FAILURE MODES — read before trusting any number I wrote.** ⛔⛔ **A RATE IS ONLY AS GOOD AS ITS SCREEN, and this is now FOUR screens I reported before validating** — the boundary one became a false priority #1, the machinery one missed `scope-facts` entirely, the falseAbsence one over-fired, and the episodic one **could not read Thai at all**. ⛔ Scripted writes destroy files (`open(path,'w')` truncates *before* writing) — use Write/Edit, or temp-file + `os.replace`. ⛔ *"Is this tracked?"* must be asked **in the directory being edited**. ⛔ Backticks in a SQL comment inside a JS template literal, 4th instance. ⛔ Test counts in commit messages before running the suite, twice.
### ⚠️ **ROOT IS HIS ACCOUNT.** Every other probe refuses it; root cells delegate to `ask-sotera-as-root.mjs`, which snapshots his room **by ID SET** (⛔ never a count) and deletes exactly what appeared. ⭐ Prefer `agent_dev` **granted** `memory_access_scope` (measured: same 5 episodes, 0 withheld, as root); toggle with `pipeline/block-vs-tools-2x2.mjs --grant/--revoke` and **leave it revoked** (it is). ⚠️ `agent_dev.display_name` is **`Claude`** — my residue, a live confound in any identity test. ⚠️⚠️ **And my harness conversations are in the corpus I measure** — now measurably, see the blocker above.
