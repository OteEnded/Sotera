...
### ⏭⏭ **RESUME HERE — 2026-08-26, 19:20. SUITE 42/42. `f11ec12` · MIGRATIONS 030–033 · RESTARTED CLEAN.** ✅✅ **THE ROADMAP'S OUTSTANDING PHASE IS DONE: CORPUS ISOLATION.** ⭐⭐⭐ **THE CORPUS NOW HAS A THIRD STATE — *“this happened, and it is not evidence”*.** Retrieval eligibility was ONE flag (`incognito = false`, **set at create, never patched**) ⇒ two states and no in-between: *never happened for any purpose* (unsettable) or *deleted* (destroys the evidence). ⛔ Everything else that LOOKS like an exclusion governs **writing**: `archived_at` → revisit lanes, `settings.probe` → noticing/reflection. **Neither ever removed a conversation from retrieval.** ⭐ `txn_conversations.excluded_from_evidence_at` + `exclusion_reason` — a **timestamp**, settable **after the fact**, which is the entire point. ⛔ `incognito` stays unsettable: a promise you can revoke later is not a promise. ⭐⭐ **ONE PREDICATE, ONE PLACE (`corpus-eligibility.js`), EIGHT READERS:** retrieve_conversations · conversation search (both arms) · relational derivation · the distiller · noticing · the reflection sweep + its backlog arm · the disclosure door. A check asserts **no reader spells a bare `incognito` clause of its own any more** — the eighth site is the one that gets forgotten. ⚠️ And the disclosure fix needed its **attribute list widened** too, or `isEvidential` would read `undefined`, treat it as *not excluded*, and pass every excluded conversation **while looking correct** — 11th instance of that family. ⛔ **033 EXCLUDES NOTHING** — not the probe runs, not `56425175`, not the three `probe:false` conversations. The capability lands; every use is a separate deliberate act, and the migration proves it excluded nothing. ⛔ `corpus.mjs`'s **two-witness DELETING contract is untouched**: the new state exists so deletion is needed **less**, never so it can be done loosely. ⏭ **NEXT:** the milestone's own line — *“let both populations grow, then reshape the schema from them — ⛔ separately, they are different instruments”* — and the corpus is now clean enough for that to mean something. ⛔ UNTOUCHED: Rome rows · lineage rows · the quarantined identity rows · Gen-3 · `56425175` · purge gate · the three `probe:false` conversations · **lexical arm OFF** · modality gate **inert** · `txn_intentions` routing **on hold** (his call) · **OteRM: a boundary we have identified, NOT started.**
