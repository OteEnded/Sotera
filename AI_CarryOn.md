...
### ⏭⏭ **WHAT TO DO NEXT: `Reference/docs/PLAN_SOTERA_WORKING_MEMORY.md` — he asked to review the plan BEFORE another large batch of code.** ⛔ Nothing in it is started. Step A (tool results become evidence) is the smallest thing that acts on the ruling; step B (Thai segmentation) is gated on a **dependency decision**; step C (working memory) is large and ⛔ must not share a batch with A or B.
