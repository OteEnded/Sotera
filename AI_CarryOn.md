...
### ⏭ **THE PROPOSAL IS WRITTEN AND UNBUILT: `Reference/docs/PLAN_SELF_HISTORY_NAVIGATION.md`.** **P1** `inspect_around` accepts the `conversationHandle` + query and resolves her message **server-side**, so cross-room ids still never leave (closes G1 ⛔ without weakening the projection) · **P2** one tool that raises the fixed card and converts the answer into a grant — **required regardless, G2 is independent** · **P3** name the next step in the payload, **last and cautiously** (prompt-adjacent, and she reached for the tool with no hint at all). ⭐ **Schema impact: NONE** — he offered a schema change and the honest answer is that the flow already has every piece of state it needs. ⏸ The one real schema question is downstream and parked: a memory *recovered under a grant* has a provenance that does not exist yet — wait for a real retention rather than predicting the column.
