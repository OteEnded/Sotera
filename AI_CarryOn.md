...
### ⏭⏭ **RESUME HERE — 2026-08-25, ~16:51. ⏭⏭⏭ NEXT IS **OTE'S CALL ON THE RETRIEVAL PAYLOAD SHAPE.** ⛔ The question is NOT *"is retrieval working"* — B4 proved it is. It is: **when `about:` matches 292 of 298 conversations, what should come back?** ⓘ Measured but ⛔ **NOT BUILT**, and nothing changes until he picks: **(a)** bound the inventory to the opened set plus counts · **(b)** put the WINDOWS first and the inventory last · **(c)** stop `notSampled` reading as an instruction to narrow. ⚠️ **He ruled in advance that this is NOT a prompt fix:** *"record that as the salience/cognition finding rather than immediately patching the prompt/tool description."* — and it is neither salience nor cognition, it is **interface shape**. ⚠️ **RESTART SOTERA EXPLICITLY and verify PID/start time before any live claim.**
