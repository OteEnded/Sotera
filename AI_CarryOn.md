...
### ⏭⏭ **RESUME HERE — 2026-08-25, ~21:05. ⏭⏭⏭ WAITING ON OTE: SHIP `windows-first`?** The comparison is DONE and it answers the architectural question: **changing the REPRESENTATION of the same retrieved information changes how well she uses it.** ⛔ **The default is UNTOUCHED** — `retrievalPayloadShape` is absent and `config.json` is **0 differences** from the pre-experiment backup. ⓘ Optional, ~8 min, blocked only by machine headroom: `plain-coverage` rep 3 and `bounded-inventory` reps 4–5 replacements. ⚠️ **RESTART SOTERA EXPLICITLY and verify PID/start time before any live claim.**
