...
### ⏭⏭ **RESUME HERE — 2026-08-26, 11:20. SUITE 40/40. `30dd951` · MIGRATIONS 030+031 · LIVE BUILD RESTARTED.** ✅ **ALL THREE DECISIONS RULED BY OTE AND IMPLEMENTED** — **A** modality: a flat five-value enum, and ⭐⭐⭐ **the SLOT is what closes** (`CHECK` in the DB + a store gate; a non-literal statement may be retained and may NOT carry `entity`/`attribute`/`value`; ⚠️ **NULL is NOT `asserted`**) · **B2** a contradicted memory leaves normal retrieval **as a WHERE clause** (incl. `findOwnLive`, so it cannot shape what replaces it) and is reachable through `recall_corrections`, with `withheldCorrections` on every read so the exclusion is a **count, not a prose marker** · **C** ⛔ no migration; act-certainty stopped flowing into `confidence` and rides as `context.actCertainty`. ⚠️⚠️ **TWO THINGS FOUND AND REPORTED, ONE FIXED:** ⭐⭐⭐ a **029 defect the check could not see** — `repl.su = kind==='identity' ? null : U` smuggled a NULL through a **bound parameter**, so the source scan for `user_id IS NULL` passed over it; kind-filtered identity search returned **0** instead of **1**. FIXED, and §4 now asserts it. ⚠️ And **`txn_memories.content_tsv` DOES NOT EXIST** ⇒ memory recall's lexical arm has been off and hybrid RRF is **dense-only**; ⛔ **NOT enabled** — it changes what recall returns while retrieval is under measurement, so it is HIS call. (`txn_messages.content_tsv` is healthy — one table, not the feature.) ⏭ **NEXT, same rule (measure first · preserve evidence · don't mutate history until the mechanism is proven): a PRODUCER for modality** — nothing sets it yet, so the gate is **armed and inert**, and the extraction prompt lives in `PortableComponents` shared with the frozen OLS platform ⇒ a conversation before a commit. ⛔ UNTOUCHED AND STAYING SO: the three lineage rows · the four Rome rows (`7d383ce3` **still live and unmarked**, asserted every run) · Gen-3 texts · `56425175` · the purge gate · the three `probe:false` conversations in his room (his ruling) · OteRM — ⭐⭐ **upgraded by Ote to *“more important, not less”***: **person · account · interface · room · relationship · memory** are six entities and **relationship assertions need provenance too**, but ⛔ **observation only, do not build.**
