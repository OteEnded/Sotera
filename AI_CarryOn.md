...
### ⏭⏭ **RESUME HERE — MIGRATION 016 IS RATIFIED AND NOT YET WRITTEN.** ⭐ **THE GAP:** three outcomes collapse into "no memory was written" — *nothing worth carrying forward* · *couldn't determine* · *found it but not authorized to retain it* — and all three are indistinguishable from **reflection never happened**. ⇒ **1. `log_reflections`**: `id · rolling_id · reflected_at · conversation_id · user_id · up_to_rolling_id · messages_considered · text NOT NULL` (her verbatim words) · `wrote_memory_id` NULLABLE (a fact, not a verdict) · `tools_used text[]` · `blocked_by_disclosure` · `prompt_generation · code_mtime · model`. ⛔⛔ **NO `outcome` ENUM** — *found-but-blocked* is mechanically observable; *nothing* vs *undetermined* is **only in her text** until repeated evidence earns a column. ⭐ **Row exists vs no row = reflection-occurred vs never-happened**, which needs no vocabulary. ⇒ **2. `txn_memories.kind` → NULLABLE**, and ⚠️ **readers must treat NULL as "she didn't say", never default it** (a default in a reader is the contamination arriving one layer later). **Proof block:** no enum type created · `wrote_memory_id` nullable · `kind IS NULL` reachable · all existing rows unchanged.
### ⏭ **THEN WIRE THE LIFECYCLE:** **quiet + changed** (⛔ there is NO conversation-end event and there cannot be — verified) → **one** opportunity per quiet stretch via `up_to_rolling_id` → **the gen-3 question VERBATIM**, ⛔ no reflection-specific slots/categories/headings → tools **offered, not required** (`recall_own_history`/`recall_own_memory`/`search_conversations`/`inspect_around`) → she decides → **the EXISTING write lane** (`buildMemoryV2(...).enqueue`, `author='persona'`, ⛔ never a second writer). ⭐ **TWO TESTS HE NAMED:** a reflection producing **no** memory **still writes a row**; a saving reflection **points at** the memory, never copies it. ⛔⛔ **REFLECTION STAYS SEPARATE FROM NOTICING** — the contaminated mechanism must not become the reflection system by convenience.
