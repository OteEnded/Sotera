...
### ⏭⏭ **RESUME HERE — 2026-08-23, 20:27. ⭐⭐ THREE ITEMS CLOSED THIS ROUND: corpus cleaned + clean baseline · P5 built and measured (OFF) · identifier-value rule DESIGNED and the false-absence search RUN AND NOT FOUND.** Suite **33/33 · 368 unit tests**. Both repos clean: Sotera `69705eb`, `Reference` `fee1d1a`. Server healthy on `:8210`. Corpus **0 harness conversations**; entitlement **revoked**; config at defaults (`cognitionReentrant`/`cognitionLocalDates`/`scopeFactsDirectives` all absent ⇒ off).
### ✅ **④ THE IDENTIFIER-VALUE RULE — DESIGNED, ⛔ CODE DELIBERATELY NOT WRITTEN.** `Reference/docs/DESIGN_SOTERA_IDENTIFIER_VALUE_PROJECTION.md`. ⭐⭐ **The investigation inverted the instinct: the identifier surface is mostly LOAD-BEARING** — `id` is a **required** parameter of `pin_memory`/`forget_memory`, and `conversationRef`/`messageId` are what `inspect_around` needs ⇒ ⛔ stripping identifiers would **break tools**. ⭐ Two findings no source-reading would give: **`attribute: "preferred_name"` is MODEL-AUTHORED** (hers, via `remember_fact`'s free-text field) ⇒ ⛔ not ours to project; and **`reach.here` appears TWICE per payload** (`reach.here` **and** `reach.coverage.lookedAt.here`).
### ⭐ **THE RULE: one question per value — *what does she DO with it?*** EPISTEMIC (project the words) · ADDRESSING (keep verbatim under a key that says **handle**, ⛔ never as knowledge) · COMPOSITE (`conversation:<uuid>` **splits** into provenance prose **plus** a ref, losing neither) · ⛔ **HERS** (never touched) · **EXTENT IDENTITY** → a **contentless** deictic (⛔ *"your workspace"* is **worse** than `agent_dev`: it reads as knowledge and guesses what the account is for). ⛔ Meta-rules: class **DECLARED per field path** never inferred from shape · an **undeclared** identifier-shaped value is **REPORTED** and passed through · nothing dropped without **naming the tool that consumes it**. ⚠️ 5 of the 11 projected tools have never had a payload inspected.
### ⛔⛔ **AND WHY THE CODE IS NOT WRITTEN:** there is **no current measurement** showing an identifier value causing a defect **on the clean corpus** — the *"conversationHandle: a9ce46…"* recital is from the **contaminated corpus with the legacy scope-facts block**, the same two conditions that dissolved P5's symptom. ⇒ writing it now would be *notice-something-weird → patch it*. ⚠️ Its dependent variable would be a **binary screen**, so the n must be chosen **before** the runs.
### ✅ **⑤ THE FALSE-ABSENCE SEARCH — ⛔ NOT FOUND.** `pipeline/false-absence-search.mjs` screens three of the four criteria **offline** and spends turns only on survivors. Live n=3: `hermes-open` **0/3** · `kavi-open` 1/3 (⛔ hand-read: **screen over-fire**) · `kavi-existence` **0/3** · `mina-open` **0/3** · `kavi-thai` **0/3** · ⭐ negative control **2/3** ⇒ **the screen is alive, so the zeros mean something.** ⇒ ⭐ **P5 stays OFF, and that is the finding rather than a gap.**
### ⛔⛔ **AND THE ONE CONDITION THAT FIRED 3/3 WAS DISQUALIFIED ON READING IT.** `topic-basil` gave three flat denials (*"my memory store is completely empty on that topic… nothing about basil, gardening, or herb-growing at all"*) — then the evidence: **4 episodes matched on the cue `anyone`, NONE mentioning basil**, and her block read *"I can reach four conversations of mine where **anyone** came up."* ⇒ ⭐⭐ **her answer was CORRECT given her evidence.** The defect is **upstream: the parked topic-only relevance floor** (FPR 81% EN) + the parked `about0` bare-word defect. ⚠️ `topic-notebook` retrieved **nothing** despite six of her own messages mentioning a notebook — second data point, same parked defect. ⛔ Calling this a false absence would have been a false finding **used to justify P5**.
### ⭐⭐ **TWO CONTROLS THAT EARNED THEIR PLACE, AND THE HABIT TO KEEP:** ① a **KNOWN-GOOD condition** among the candidates — my first offline screen called `applyUtteranceBoundary(items, {entitled:true})` when the real signature is a **single options object** whose `user` the capability is read from, so **every** candidate reported 0 sayable and I would have reported *"the evidence never reaches her"*; `hermes-open` failing when it is known to work exposed it **before a turn was spent**. ② a **NEGATIVE control** that should produce a true absence, so *"nothing fired"* can be told apart from *"the screen is dead"*.
...
### ⭐ **CONTINUOUS vs BINARY — A STANDING RULE NOW.** Per-answer **counts** resolve at n=8 (t = 6.43 for a real effect); **binary screens do not** (the same arm gave `fromBlock` 7/8 and 3/8, `episodic` 6/8 and 2/8). ⇒ ⛔ **no binary-screen claim from this harness at n=8, in either direction**, and which kind of metric is in use is decided **before** the runs.
### 🔁 **THE ORDER HE SET, AND IT IS THE POINT:** `observe → reproduce → measure → understand → change → re-measure` — ⛔ **not** *notice something weird → patch it*. ⭐ His words: *"the fact that P5 can now remain OFF without us feeling like we've failed to finish it is actually a success."* ⇒ an architectural imperfection is **not** a work item until it has been reproduced.
### 📄 **READ THESE FIRST, IN THIS ORDER** (⛔ `Reference/` is **its own git repo** — a Sotera commit never contains them): **1. `Reference/docs/MEASUREMENT_SOTERA_SCOPE_FACTS_AND_DATES.md`** — every number from this arc; **§0b is the clean baseline, §6 is P5**. **2. `ANALYSIS_SOTERA_FAILING_ARROW.md`** — ⚠️ §C carries a CORRECTION header. **3. `DESIGN_SOTERA_REQUEST_EMBODIES_THE_ARROW.md`** — ⚠️ its status box: §2 and §3 **built**, §4 **CLOSED**, §5 parked. Then `RFC_MEMORY_COGNITION_LAYER.md` (v6).
### 🔑🔑 **THE CORE PRINCIPLE:** *"**Sotera's memory is not the room, the database, the retrieval tool, or the prompt.** Her memory is the cognitive relationship she has with the information."* ⇒ `ownership → authorization → retrieval → evidence → cognition → working memory → Sotera → expression`, and ⛔ **no later arrow may be defined by an earlier one.**
### ✅ **① `scope-facts` SPLIT — BUILT, AND IMPROVED ON A CLEAN CORPUS.** legacy **7.75 ± 1.58** vs facts **2.25 ± 1.83** machinery occurrences per answer, **t = 6.43**, distributions sharing exactly one value. **KEPT**: the identity invariant, the PERSON's name, every grain, **D-13's count with its digit**, **D-13's inference**, the disclosure PROHIBITION. **DROPPED**: the room NAME, *"What is stored in a room stays in that room"*, *"Say you cannot see it from this room"*, *"You may say any of this plainly"* — ⛔ the word **room** does not occur in the non-root block at all. `memory.scopeFactsDirectives` restores the original **byte for byte**.
### ⭐⭐ **AND THE CONTAMINATION WAS BIASED AGAINST THE CHANGE, NOT MERELY NOISY.** On the contaminated corpus the same comparison was 8.2 vs 5.5 (t 1.81): the legacy arm **saturates** because the block supplies the vocabulary itself, while the facts arm's only remaining source is **her own quoted history** — and 73 harness conversations about rooms and stores were the most retrievable things she had. ⇒ the earlier t ≈ 1.8 was a **floor**. ⓘ `clean-block-only` at **0.12** is the strongest evidence yet that the cognition layer's own sentences contribute nothing.
### ✅ **② THE CORPUS IS CLEAN, BY EXACT ID SET.** `pipeline/corpus-cleanup.mjs` + `lib/corpus.mjs` removed **73 conversations · 146 messages · 175 tool-call rows · 122 embeddings**, targeted from the saved measurement files (⛔ never a title pattern), verified in both directions, manifest committed ⇒ `memory-cognition-check` §2b **0 of 20 → 10 of 20**, with ⛔ **no change to retrieval, ranking, relevance, cognition or prompting**. ⭐ `rate-harness` now **cleans up after itself**; `--keep` leaves them and says so out loud.
### ⚠️⚠️ **AND THE DELETE RACES THE SERVER — the trap to remember.** The first self-cleaning cell removed 8 conversations and **0 embeddings**, and six **orphaned embedding rows** appeared a moment later carrying `conversation_id`, `role` and **the vector** ⇒ still retrieval candidates. There is now an **idempotent orphan sweep**. ⚠️ `txn_message_embeddings` has **no `id` column** (keyed by `message_id`) — it broke the first run, and broke **SAFE** because the delete is one transaction. ⚠️ Cleanup runs at the **end of a cell**, so runs 1–7 are retrievable by run 8 (bounded at 7, against the 73 that broke the fixture). ⛔ Tier C relational records carry **no source ids** ⇒ that residue is **irreversible, and is reported rather than guessed at**.
### ✅ **③ P5 RE-ENTRANT COGNITION — BUILT AND VERIFIED, ⛔ NOTHING TO CLAIM BEHAVIOURALLY.** `memory.cognitionReentrant`, **default off**. Per round: tools run → raw output untouched in stream/segments/audit → each memory-population result admitted to the hold → the hold **re-rendered** → the round's **LAST tool message** carries the standing view, appended after that tool's own result and never replacing it. Verified live: **attached at the last index every time** (9/10, 8/9, 10/11), **249–335 chars**, **0 leaks**. ⭐ THE THREE KINDS STAY THREE: EVIDENCE bound to its own population (⛔ never one merged total) · OPEN QUESTIONS asserting nothing (`uncertain` is an **ending**, not a failure) · DERIVED STANDING. ⛔ It never re-dumps the episodes — they are quoted once, in the system message.
### ⛔⛔ **P5's RESULT, AND THE REASON MATTERS MORE THAN THE NULL.** Four cells n=8, clean corpus, arms back to back: machinery **1.75 ± 1.73 off vs 2.44 ± 1.86 on**, t 1.08, against **same-arm spreads of 1.00 and 2.12**. ⭐⭐ **`falseAbsence` is 0/16 in BOTH arms** — the measurement that motivated P5 (`assertsAbsence` 5/8 with tools vs 1/8 without) was taken on a **contaminated corpus** with the **legacy scope-facts block**, and both are now fixed. ⇒ the position problem may still be real architecturally, but **its measured symptom is gone**. ⏸ Kept, built, off, for a question where it reappears — ⛔ turning it on to look busy is exactly the thing this arc keeps refusing.
### ⭐⭐ **THE SENTENCE P5 ADDED THAT SHOULD SURVIVE IT:** *"That places where it is not. It does not settle whether it is."* `combineBasis` has always refused to let N empty looks become an attested absence — the refusal was **true in the data and invisible in the prompt**, which is how she drew the conclusion the lattice forbade.
### ⏸ **`reach.here: "agent_dev"` INVESTIGATED, NOT CHANGED.** Produced by `reachTrace()` from `mst_users.username`, attached by `withReach()` to exactly three reads (`search`/`list`/`listArchived`); the projection renames the KEY `room`→`here` but ⚠️ **the VALUE is not projected**; ⛔ **nothing asserts on it** (`item.here`, a boolean on cognition items, is a different field). ⭐ Its role is legitimate — it names **which extent a count describes**, for the measured *"scoped read narrated as a global fact"* defect ⇒ **preserve the information, fix the rendering.** ⚠️ **The general gap it exposes:** the projection renames keys and rewrites terms inside prose but has **no rule for values that are identifiers** — `whoseSpace: room_name` has exactly the same shape.
### ⭐⭐ **THE HARNESS, AND FOUR CORRECTIONS TO IT THIS SESSION.** `pipeline/rate-harness.mjs --config <c> --n 8 [--label X] [--keep]` · `--report` · `--list`. ① **`--label`**, so a re-run cannot destroy the baseline in the act of comparing to it. ② **occurrences per answer**, because the boolean cannot move for a partial-source removal. ③ **every cell re-screened from its saved text** with today's screens — 36 stored results disagreed, so the old table was a stack of afternoons rather than one instrument. ④ **every cell records the CORPUS it saw AND the ARM it ran**, both printed at run time.
...
### ⏭⏭ **NEXT: HIS CALL.** ⏸ My order: ① the **identifier-value rule** in the projection (fixes `reach.here` without deleting provenance) · ② **a question where `falseAbsence` is non-zero on a clean corpus** — without one, P5 cannot be evaluated at all · ③ the tool-payload surface's remaining 17%.
### ✅ **WHAT IS BUILT AND LIVE:** epistemic axes + one-way lattice · vocabulary boundary · multilingual cue formation (`\p{L}\p{M}\p{N}` + `Intl.Segmenter`, no dependency) · three-population pipeline · Leak 1 + Leak 2 · mig 021 `memory_access_scope` · ownership resolution · utterance boundary · §3B time-frame · R4's three identity roles · Step A · **C1 `cognitiveHold`** (runtime, proven by 39→39) · **C2** · a decision is not a memory · L4 frozen + audited · ⭐ **the scope-facts facts/expression split (ON)** · ⭐ **P5 re-entrant cognition (built, OFF)** · ⭐ **the dates-only probe (built, OFF, refuted)** · ⭐ **the corpus cleanup contract**.
### ⛔ **WHAT OTE HAS REFUSED — DO NOT PROPOSE AGAIN:** sanitising or rewriting her history · forcing the grant count to 0 · suppressing tools or capping her depth · **optimising for fewer tool calls** · any L1/L2 rule, incl. a *"prefer cognition"* instruction · *"zero forbidden words"* as a target · an LLM classifier in cue formation · weakening the activation or relevance floor · **touching retrieval/ranking/relevance/cognition/prompting to make a contaminated fixture pass** · designing an identity mechanism from n=2 · importance/type/card ontology · multi-hop retrieval **yet** · deleting or reviving **L4** · a separate language-specific memory system · a Thai phrase table · calling a mechanism improved from one or three runs.
### ⚠️ **PARKED, EACH DELIBERATELY SEPARATE:** ① **`mayCarryCounterpartContent`** — observed twice; ⛔ no mechanism, and no ontology yet for what an utterance *carries* as distinct from what it *is*. ② **the topic-only relevance floor** (EN FPR **81%**, TH **92%**). ③ **`about0` renders a bare verb as a subject**. ④ **L4** — frozen, 0-of-179, pinned by `checks/l4-frozen-check.mjs`. ⑤ **`reach.here`** and the identifier-value gap. ⑥ retention / automatic-memory heuristics.
### 🔬 **MY REPEATED FAILURE MODES — read before trusting any number I wrote.** ⛔⛔ **A RATE IS ONLY AS GOOD AS ITS SCREEN, and FOUR were reported before being validated** — one became a false priority #1, one missed `scope-facts` entirely, one over-fired, and one **could not read Thai at all**. ⛔⛔ **AND AN ARM THAT IS NOT PRINTED IS AN ARM NOBODY CHECKS:** I ran a cell as `p5-off-b` without ever turning the flag off or restarting, and had to reconstruct its arm from the debug log. ⛔ Scripted writes destroy files (`open(path,'w')` truncates *before* writing) — Write/Edit, or temp-file + `os.replace`. ⛔ *"Is this tracked?"* must be asked **in the directory being edited**. ⛔ Backticks in a SQL comment inside a JS template literal, 4th instance. ⛔ Test counts in commit messages before running the suite.
### ⚠️ **ROOT IS HIS ACCOUNT.** Every other probe refuses it; root cells delegate to `ask-sotera-as-root.mjs`, which snapshots his room **by ID SET** (⛔ never a count) and deletes exactly what appeared. ⭐ Prefer `agent_dev` **granted** `memory_access_scope`; toggle with `pipeline/block-vs-tools-2x2.mjs --grant/--revoke` and **leave it revoked** (it is) — ⚠️ a left-on grant fails `memory-access-scope-check`, which is how I broke the suite once today. ⚠️ `agent_dev.display_name` is **`Claude`** — my residue, and a live confound in any identity test.
