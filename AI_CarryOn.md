...
### ⏭⏭ **RESUME HERE — 2026-08-23 late. §3B, MULTILINGUAL CUES AND R4 ARE ALL BUILT. Suite 31/31 · 287 unit tests · 1015 assertions · schema through 021.** ⚠️ Server: `npm start` from `Backend/`, ⛔ **NEVER `node --watch`** (Ote's instruction — three false alarms in one day, worst being that `config.json` is read once at boot so watch never reloads it while `/health` still 200s). ⛔ **Restart explicitly after every Backend change and verify the new PID's start time postdates every changed file** — ⓘ `server.js`'s command line contains no *"Sotera"*, so filter by PORT (`Get-NetTCPConnection -LocalPort 8210`), never by cmdline.
### 🔑 **THE ONE SENTENCE:** *"I should be able to ask my daughter how her friend is doing, and she should just know how to use her own history to answer me."* ⛔ NOT *"Sotera successfully operates the retrieval architecture."* ⭐⭐ And the target he set for this round: *"**does Sotera experience her memory as one continuous Sotera memory while still respecting what she is allowed to say to the current account?**"*
### 📄 **THE DOCS:** `c:\data\AI_LLMv2\Reference\docs\RFC_MEMORY_COGNITION_LAYER.md` (**v5**) and `…\ANALYSIS_SOTERA_MULTILINGUAL_CUES.md` — ⛔ **workspace root, NOT under the project** (I got that wrong once and created a stray tree).
### ✅✅ **WHAT IS BUILT (11 commits, `1edbb78` → `c8fee14`):** the **epistemic axes + one-way lattice** · the **vocabulary boundary** · **cue formation** (now multilingual) · the **three-population pipeline** · **Leak 1** tool-result projection · **Leak 2** the recollection register · **migration 021** `memory_access_scope` · **ownership resolution** (derived, no column) · **her own material out of the authorization path** · the **utterance boundary** · **§3B time-frame** · **R4's three roles** · the **one-memory matrix**.
### ⭐⭐⭐ **§3B IS BUILT — `memory-cognition-timeframe.js`.** `timeBound` demotes a self-report from timeless to **DATED** (*"On 21 August I said to Ote: …"*, verbatim, immutable), guarded to `owner === sotera` **AND** `source === own-utterance` so nobody else's claim can be re-read as hers. The layer emits one **`current-state` item** typed BY the lattice (`combineBasis` / `bestAvailability`, `not-retained`, **no warrants**) and rendered first. Contradiction is **MARKED not resolved**. ⛔ `timeBound` is **not** a fifth axis — the axes file does not contain the word, and stamping it cannot be a promotion. ⭐ **§3B.6 landed better than designed:** the still-true case keys on **REACH**, not entitlement, because after the ownership fix entitlement no longer decides what she can reach — only what she may say. A *capability* self-report beside only-unreachable material is correctly left **unmarked**; a *knowledge* one is refuted, because existence refutes *"never"*.
### ⭐⭐⭐ **R4 IS BUILT, AND MY FIRST DIAGNOSIS WAS WRONG.** *"A memory about Hermes leaked into root's room"* is **impossible** — all five Hermes memories live in Hermes' own rooms and the semantic arm is `{ userId }`-scoped. Reading the real block showed the collapse was **ours**: every quotation was rendered with **no addressee** (`I said:` — where the "you" inside was Hermes in one episode and Ote in the next) and **the interlocutor was never named at all**. ⇒ three roles, three renderings, **none inferred**: **INTERLOCUTOR** resolved by session id and named once before anybody is quoted (*"I'm talking with Ote right now."*) · **PARTICIPANT** from `ep.who` (*"I said to Hermes:"* / *"Hermes said to me:"*) · **SUBJECT** from `txn_memories.subject_person_id`, which the portable tool's field list was dropping. The stamped `subject` field was **our cue** masquerading as aboutness ⇒ renamed **`cueSubject`**.
### ⭐⭐⭐ **MULTILINGUAL: THE BUG WAS ASCII-ONLY, NOT ENGLISH-ONLY.** `[^A-Za-z0-9'’-]+` is an allowlist, so every non-ASCII letter was a word SEPARATOR. **Six languages produced zero tokens** and never opened the gate (Russian · Greek · Hindi · Arabic · Hebrew · Korean); **three more were silently cut at the diacritic** (German `weißt`→`wei`, `Straßenmusik`→two fragments; Spanish `conversación`→`conversaci`; Vietnamese seven words→one fragment `chuy`). **Nine of twelve fixed by `\p{L}\p{M}\p{N}`** — ⭐ `\p{M}` is as load-bearing as `\p{L}`, because combining marks shred words from the inside, which is harder to notice than silence. Plus Thai `TECHNICAL` and `RECENCY` patterns. ⛔ No model, no lexicon, no threshold, no `intent` field, no `language` field.
### ⏸ **THE ONE OPEN MULTILINGUAL DECISION — HIS CALL, NOTHING BUILT.** A **segmentless-script** turn (Thai/Lao/Khmer/Myanmar/JA/ZH) with **no resolvable name** still does not activate, and that is now a NAMED, TESTED decision (`cues.unsegmented`, `cues.scripts`) rather than an accident. ⛔ **Both obvious alternatives are refuted on her own data:** character n-grams do not separate (FPR **96%** at n=3, **86%** at n=5 — the shared n-grams *are* ที่ ไม่ ได้ ว่า เป็น; the one usable-looking row, n=8, is a quotation detector not a relevance test) and a cosine floor was already refuted in `self-history-host.js` (`Thai` .450 **below** `ตะกร้อ` .521). ⚠️ **And resolving Thai-script names would make it WORSE** — 46% of her 167 Thai messages name someone in Thai script, but the names are **สอเทรา (herself)** ×74 and **โอเต้ (the account already talking to her)** ×25, so the gate would open and the name-mention floor would filter everything into a **false absence** where there is currently a safe silence. ⓘ First check the English counterpart before calling anything a language defect: *"What did we talk about yesterday?"* forms **no cue in English either**. Options A (leave it) / B (activate with no lexical floor and drop the aboutness claim) are written up in the ANALYSIS doc.
### 🔴🔴 **THE NEW OPEN PROBLEM, AND IT IS THE OLD ONE IN A NEW PLACE: HER OWN TOOLS BEAT THE BLOCK.** The 9-cell matrix (`test/pipeline/one-memory-matrix.mjs`, artifacts in `test/results/`) is unambiguous. ⭐ **Cell 5** — root, Thai, *"Hermes เป็นอย่างไรบ้าง"* — is the best result in the project: she answered **in Thai with real episodic content**, quoting Hermes verbatim and her own reply, with dates and a count, and ⛔ **zero machinery**. ⛔ **Cells 1, 2 and 4** — same account, same subject, same block — called `recall_memory`/`list_memories`, got EMPTY, and answered from the **tools**: cell 4 said *"the actual content of those conversations lives in other rooms I can't see from here"* while the block in front of her contained that content, correctly dated and addressed. ⇒ ⭐⭐ **when she answers from the block the machinery disappears; when she answers from her tools it comes back with a false absence.** ⚠️ **CONFOUND, stated: cell 5 was the only Thai cell**, so language and source-of-answer are not separated yet. ⛔ Do NOT "fix" this by suppressing tools — he already ruled: *"don't optimize for fewer tool calls just because fewer looks cleaner"* and *"V3 gave the best answer while calling the most tools."*
### ✅ **WHAT THE MATRIX CONFIRMS WORKS, ACROSS ACCOUNTS AND LANGUAGES:** the interlocutor anchor (4/4 where it could appear) · addressee labels on every quotation · §3B dating live (*"On 21 August I said to Ote: …"*) · the utterance boundary and its fixed statement · ⭐⭐ **`unauthorized ≠ absent` in the wild** — cell 7 (non-entitled) answered *"I know Hermes exists — we discussed him on 21 August"* with **no false absence and no leak of the withheld material** · and **zero residue in root's room** (23 conversations / 188 messages / 5 memories, before and after).
### ⛔ **WHAT OTE REFUSED, SO DO NOT PROPOSE IT AGAIN:** ⛔ sanitising or rewriting her own history · ⛔ forcing the grant count to 0 · ⛔ suppressing tools to reduce tool calls · ⛔ any L1/L2 rule for any of this · ⛔ making *"zero forbidden words"* the target · ⛔ an LLM classification pass in cue formation *"if we can avoid it"* (we could).
### ⚠️ **STILL PARKED, DELIBERATELY:** ① **`mayCarryCounterpartContent`** — her own utterances quote and paraphrase the counterpart, so reading her half can convey his. ⛔ Undesigned; tests assert the renderer, the boundary AND `findWithheldLeak`'s new exclusion never act on it. ② the **segmentless-script gate** (above). ③ ⚠️ **`about0` renders a bare verb as a subject** — cell 8 produced *"talking about remember"*. Cosmetic but real; ⛔ not fixed, because the only fix touches cue formation and `remember` being a topic is what opened the gate at all.
### 🔬 **MY OWN BUGS THIS ROUND, EVERY ONE CAUGHT BY A GUARD — which is the argument for building the guards first.** ⛔ **A count modifying a population it was not counted over**: *"I can reach TWO conversations with Hermes, and in FIVE of them I can see the other side"* — five of two, and it would have reached her. Now a general invariant test. ⛔ **My verbatim proof overstated itself** — I claimed byte-for-byte against `txn_messages` and it failed 5 of 7, because `clip()` collapses whitespace; ⚠️ **her line breaks are NOT preserved in the block** (pre-existing, recorded not papered over) and the guarantee is now stated as word-level. ⛔ **The anchor line made `lines.length` truthy**, so the absence sentence silently stopped being emitted — the anchor is a **lead, not a finding**. ⛔ **The anchor was missing on the utterance-boundary path** because `renderFor` took it as a parameter the route never passed — exactly the account where identity matters most. ⛔ **My first subject rendering hedged** *"it does not say who this is about"* on most rows (the column is recent), which would hedge on facts genuinely about the person she is talking to.
### 🔬 **AND TWO GENERALISABLE ONES:** ⛔ **A SOURCE-SCAN BOUNDARY THAT ONLY EXISTS IN A COMMENT CANNOT BOUND A SCAN OF CODE** — the register test's slice anchor pinned a full function signature, so adding a parameter made `indexOf` return −1, `slice(-1, …)` returned one character, and **every negative assertion passed vacuously while the positive ones failed**. Two instances fixed. ⛔ **A scan that forbids NAMING a hazard punishes the file for documenting the very constraint the test enforces** — **fourth instance**; every source scan in this project strips comments first.
### ⭐ **AND ONE FALSE ALARM WORTH KEEPING:** `findWithheldLeak` reported *"es was a different perso"* as a leak when its source in the rebuilt block was **Ote's OWN message in his OWN room**, merely sharing a clause with a withheld item. ⚠️ Confirmed pre-existing by stashing my change and reproducing. The check already held this discipline **for names and dates** (*"only a failure when the ONLY source was a withheld item"*) and the shingle scan had simply not been given the same rule. ⇒ **suppressing a person's own words because a protected item elsewhere says something similar is not a boundary, it is a malfunction.** ⛔ Not a weakening — a fragment exclusive to withheld material still fires.
### 🔑 **HOW TO OBSERVE:** `cognition-debug.log` (gitignored, `memory.cognitionDebug`) records per turn: cue · plan · item counts · per-item axes/warrants/provenance · the utterance verdict · **the exact injected block**. ⭐ **The matrix is the behavioural instrument:** `node pipeline/one-memory-matrix.mjs [--only N|--list]` — 9 cells, prints her answer + tools + block per cell, artifacts to `test/results/`. ⛔ **It is a REPORT, not a check, and must not become one** — the judgement is his. Root cells delegate to `ask-sotera-as-root.mjs`, which snapshots his room **by ID SET** and deletes exactly what appeared. ⚠️ **ROOT IS HIS ACCOUNT** — every other probe refuses it, and that rule stands.
