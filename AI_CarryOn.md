...
### ⏭⏭ **RESUME HERE — 2026-08-25, ~19:28. ⏭⏭⏭ WAITING ON OTE, ⛔ NO GPU UNTIL HE ANSWERS.** The three payload shapes are **built, gated and measured offline**; the live comparison is **NOT RUN**, and it is blocked by a **CEILING, not a cost**: with `current` scoring **5/5 three times out of three**, ⛔ no shape can win on correctness and the 8-run design cannot rank them. ⭐ **THE TWO OPTIONS PUT TO HIM:** **(a)** run the comparison on **efficiency + axis quality** with correctness as a FLOOR (must stay 5/5, must not invent on the control) — the control distribution exists: **tools 11–12 · retrieval 8–10 · badAxis 0–1 · wall 96–134 s · prompt 65k–75k**; or **(b)** build a HARDER target first so correctness discriminates again. ⚠️ Neither can measure whether a shape removes the **intermittent** 0/5 — that needs many runs and the number is his. ⚠️ **RESTART SOTERA EXPLICITLY and verify PID/start time before any live claim.**
