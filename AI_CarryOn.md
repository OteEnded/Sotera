...
### ⏭⏭ **RESUME HERE — 2026-08-26, 18:40 +07. THE BOX FELL OVER AND CAME BACK; THE CURSOR PROOF IS STILL UNSPENT.** ⛔⛔ **OUTAGE 18:21:07 +07 — WINDOWS LOW *VIRTUAL MEMORY*, AND IT IS THE SECOND TIME THIS BOX HAS HIT ITS COMMIT CEILING.** The event names them: `llama-server.exe(18900)` **29.8 GB** + `llama-server.exe(16112)` **8.5 GB** + `claude.exe` 2.4 GB against a **71.7 GB** commit limit. **Postgres was KILLED** — its log ends mid-traffic at 18:21:06 with **no shutdown line** — and the VS Code window died with it. ✅ Recovery was clean: `database system was not properly shut down` → **redo done in 0.01 s**, no PANIC, no FATAL, counts intact (107 memories · 1,970 messages · 384 conversations · 74 revisits). ⚠️ **RESTARTING IT NEEDS ELEVATION** — the service runs as `NT AUTHORITY\NetworkService`, and `net start postgresql-x64-17` from an unelevated shell is **System error 5**; the working move is `Start-Process net -ArgumentList 'start','postgresql-x64-17' -Verb RunAs`, which puts a UAC prompt in front of Ote. ⭐⭐ **THE FINDING WITH A CONSEQUENCE: CONTENTION HAS A SECOND AXIS, AND IT IS NOT THE GPU.** I had been framing the contention experiment entirely as *does a background 35B steal the GPU from an interactive turn* — ⛔ **that is the narrow half.** A 23 GB model resident on the cards still commits host address space, and **two** resident models took down the database and the editor. ⇒ the A/B must measure **host commit** alongside TTFT, or it will certify a lane that is polite about GPU scheduling and still fatal. ✅ **AND THE GUARD IS BUILT** (`test/lib/host-commit.mjs` + `checks/host-commit-check.mjs`, 25 assertions, `b98e94e`): a streaming commit probe on its **own process** (⛔ never sampled inline in the latency path), per-sample commit on every result, a **95% mid-run abort** that releases the background load first, and a preflight with **five distinct refusal reasons** — `requirement-unknown` · `commit-unreadable` · `commit-pressure` · `insufficient-headroom` · `projected-pressure`. ⭐ The requirement is **derived from what is actually resident**, ⛔ not a magic number: resident ⇒ the run adds a **slot** (8 GB reserve), absent ⇒ it **loads weights** (reserve + real size from `/api/tags`). ⛔ Ollama unreadable ⇒ requirement unknown ⇒ **refuse**. ⚠️⚠️ **AND THE FIRST `--dry` CAUGHT THE GUARD PERMITTING THE STATE IT FORBIDS** — 54.8% committed, 32.4 GB headroom, 30.3 GB required ⇒ PASS, while loading the 22.3 GB model would reach **85.9%**, above its own **85%** ceiling. ⭐ The week's shape once more: **ONE THRESHOLD ANSWERING TWO QUESTIONS** — *safe now* and *safe after*. The ceiling now applies to the **projected** commit too. ⓘ Consequence worth knowing: **from cold this experiment cannot fit inside its own ceiling** (22.3 GB of weights on a 71.7 GB limit is 85.9% unaided); it passes only with the 35B **already resident**, which is also the realistic contention condition. ⚠️ And the **commit limit is not a constant** — 71.7 / 73.2 / 79.7 GB observed within twenty minutes as the system-managed pagefile grew and contracted, so a cached limit silently misreports. ⚠️ **AND IT ATE A REAL TURN:** `7198c1b0` is **Ote's own room** (`probe:false`); his message at **18:19:04** is followed by `llama-server` spawning at 18:19:07, the ceiling at 18:21:07, and **no assistant reply at all**. ⭐ A dangling user message is the user-facing shape of this failure, and he regenerated it himself once the box was back. ⚠️ **MY OWN MISREAD, CORRECTED:** I reported *':8301 not listening ⇒ the app is down'*. **Sotera is `:8210`** — I checked a port it has never used. The app (pid 34664, up since 16:31) **never died**, and its pg pool **reconnected by itself** once the service returned; `agent_dev` login → 200. OLS `:8201` was untouched throughout. ✅ **THE REFLECTION LANE IS ALIVE AND THE LIFECYCLE IS REAL** — six attempts `#652`–`#657`, **all `completed`, zero preempted, no user message inside any run window**. ⭐ `from = (start)` on all six is **CORRECT, not a cursor failure**: none of the six carried a prior watermark, so there was nothing to continue from. ✅✅⭐ **GPU PLACEMENT IS PROVEN BY LIVE EXECUTION** (`pipeline/reflection-placement.mjs`, `f0031f3`). Ote: *“absence from VRAM while idle does not prove the worker's actual placement.”* ⭐ Right, and the two facts are now separate: **CONFIGURED** — the lane passes no residency option and the `ollama` provider has no `forceCpu`, so no `numGpu` is injected ⇒ GPU. **OBSERVED** — one controlled run on `1be99117`: `qwen3.6:35b` **23.05 GB in VRAM**, **720 MiB → 24,009 MiB**, peak **98%** across **both** RTX 5060 Ti, **33.3 s**. ⛔ Deliberately **not** run on `7198c1b0`: forcing a run there would have consumed the proof artificially. ⏳ **THE CURSOR PROOF IS DEFERRED, NOT SPENT.** `7198c1b0` holds watermark **4920** with ~**149 unreviewed** — the only conversation that can prove continuity without anyone typing. ⚠️ It is a LIVE room, so every new message restarts the 30-minute quiet gate; that is the gate working, not a fault. ⭐ The decisive check is unchanged and is his: **`from_rolling_id = 4921`. If it does not produce that, characterize the failure — do not manufacture or repair the cursor.** Then the `56425175` exclusion (dry-run already clean), then the contention A/B. ⚠️ **FIVE DEFECTS IN MY OWN INSTRUMENTS, all caught before they became findings:** ①⛔⛔ **`import()` IS NOT A PARSE CHECK** — the habit adopted after two outages parses a module that only *defines*, and **silently executes a script that *does***: it logged in, opened a conversation and drove ~5 turns. ⭐ **`node --check`** is the tool. ② a **noise floor that was one draw of the noise** (`|median(A)−median(B)|`, N=6) ⇒ **pooled IQR**. ③ **`rolling_id` is a GLOBAL sequence** — `max − watermark` reported **5,520 unreviewed for a 4-message conversation** ⇒ backlog is COUNTED. ④ **two panels of one instrument contradicted each other on screen** — `7198c1b0` annotated *ELIGIBLE* while 24 minutes into a 30-minute gate ⇒ the annotation now names the specific reason. ⑤ **I printed UTC as if it were his clock** ⇒ **+07** from here. ⛔ Unchanged: Rome rows · the quarantined identity rows · Gen-3 texts · the three `probe:false` conversations · priors **OFF** · lexical arm **off** · modality gate **inert** · 033 excludes **nothing** · OteRM **not started**.
### ✅ **EARLIER 2026-08-26 (superseded as RESUME) — 2026-08-26, 22:00. SUITE 42/42. `60ab8c1` · RESTARTED CLEAN.** ✅ **① “IS SOTERA BUSY?” IS NOW A ROOT PLUGIN** (`app/plugins/steer-registry.js`, `fp`, registered **before** the cron that reads it). The chat route **consumes** `fastify.steerReg` and no longer imports the factory — ⛔ its absence is the point. **Still exactly ONE registry.** ⭐⭐ **AND THE REASON CODE IS SPLIT:** `GATE.registryAbsent` is its own value. Both branches fail closed and that was right, but for 28 h the log said `config-unreadable` while the config was fine. ⇒ the week's overload, in an **ERROR PATH**, where it is hardest to notice. ⚠⚠ **AND THE TEST THAT SHOULD HAVE CAUGHT IT PASSED THE WHOLE TIME** — it asserted the literal line `fastify.decorate('steerReg', steerReg)`, which really was there, **on a child scope no sibling could see**. ⭐ **It asserted the LINE, not the PROPERTY.** Now it asserts: created in an `fp` ROOT plugin · registered before cron · route creates none. ✅ **② THE NOTICING POPULATION IS CHARACTERIZED MECHANICALLY** (`pipeline/noticing-shape.mjs`, `Reference/docs/MEASUREMENT_SOTERA_NOTICING_SHAPE.md`) — ⛔ nothing read for meaning, nothing classified, nothing excluded. **111 gen-3 rows · 188,766 chars · median 1,647 · all `unclassified`.** ⭐ **111/111 distinct by hash, 0 byte-identical, 0 repeat `(conversation,upTo)`** ⇒ the pass never asked twice about identical material. Rooms **Hermes 46 · Ote 33 · Claude 32**; 33 conversations, ⚠️ **top-5 hold 50%**. ⭐⭐⭐ **THE FINDING WITH A CONSEQUENCE: `priorLessonsOffered = 0` ON ALL 111** — she has **never been shown a prior proposal**, so *“does the same distinction recur”* is **structurally unanswerable from her side**. ⚠️ Turning priors on is **not free**: it puts her own prior vocabulary in front of her, which is the contamination shape that already withdrew one finding. ⭐⭐ **AND NO CONTAMINATION: ZERO rows from `probe:true` conversations** — the pass has been reading real conversations throughout. ⚠️ But **7 rows point at conversations since DELETED** ⇒ the concrete case for 033: exclusion would have kept both. ⚠️ **08-22 is missing entirely** (59 h gap). ⏭ **NEXT DECISION:** whether to turn **priors** on for the noticing pass — it is the only way her side of *“does it recur”* becomes answerable, and it is a **contamination trade**. ⛔ UNCHANGED: Rome rows · lineage rows · quarantined identity rows · Gen-3 texts · `56425175` · purge gate · the three `probe:false` conversations · **lexical arm OFF** · modality gate **inert** · 033 excludes **nothing** · **OteRM NOT started**.
