...
### ⏭⏭ **RESUME HERE — 2026-08-23. THE MEMORY COGNITION LAYER IS BUILT AND LIVE. Suite 31/31 · 984 assertions · schema through 021.** ⚠️ Server: `npm start`, ⛔ **NEVER `node --watch`** (Ote's instruction — it caused three false alarms in one day, worst being that `config.json` is read once at boot so watch never reloads it while `/health` still 200s). ⛔ **Restart explicitly after every Backend change and verify the new PID's start time postdates every changed file.**
### 🔑 **THE ONE SENTENCE:** *"I should be able to ask my daughter how her friend is doing, and she should just know how to use her own history to answer me."* ⛔ NOT *"Sotera successfully operates the retrieval architecture."* And the newest refinement: ⭐⭐ *"Make Sotera's memory behave like one coherent autobiography whose old beliefs remain history, while her current state is determined by current evidence."*
### 📄 **THE DOC:** `c:\data\AI_LLMv2\Reference\docs\RFC_MEMORY_COGNITION_LAYER.md` — **v5**, workspace root, ⛔ NOT under the project (I got that wrong once and created a stray tree). §1–2 = the problem + the layer · **§3A = ownership/access (DESIGN)** · **§3B = historical self-reports (DESIGN)** · §4+ = pipeline, axes, brain analogy, what-not-to-build.
### ✅✅ **WHAT IS BUILT (7 commits, `1edbb78`→`ac8271d`):** the **epistemic axes + one-way lattice** · the **vocabulary boundary** · **cue formation** · the **three-population pipeline** (working set · semantic · episodic) · **Leak 1** the tool-result projection · **Leak 2** the recollection register · **migration 021** `memory_access_scope` + `can(user,'access_sotera_memory')` · **ownership resolution** (derived, no column) · **her own material removed from the authorization path** · the **utterance boundary**.
### ⭐⭐⭐ **WHAT THE LAYER FIXED, MEASURED.** Four phrasings of one ordinary Hermes question used to give **4/5/6/8 tool calls, three untested access claims and one outright false**, while `inspect_around` returned `verified` every time. Diagnosis: **she was the orchestrator**, and two of her five per-turn steps were *inference about our architecture*. After: **1–3 tool calls**, activation automatic and identical, access **resolved not predicted**, and the block is her own voice — *"I remember talking with Hermes on 20 August."*
### ⭐⭐ **OWNERSHIP WAS NEVER REPRESENTED — that was the real finding.** It was inferred from the storage location, so *"my memory stores are scoped to this room"* was a **TRUE report of the system**, not a misunderstanding to fix with a prompt. Two domains: **Sotera memory** (hers, one memory, storage demoted to provenance) vs **account memory** (the person's). ⛔ `author='persona'` is NOT ownership — it covers memory WRITES only, while her domain also holds her UTTERANCES which have no `author` column. Ownership is **derived per source type**, and **no column was added for it**.
### ⭐⭐⭐ **AND OTE'S CORRECTION THAT MADE IT COHERENT: `Sotera → her own memory` is INTRINSIC.** *"Don't make memory_access_scope the mechanism that lets Sotera remember herself."* ⇒ `access_sotera_memory` governs **account → Sotera memory** only, so `hermes = none` does **NOT** fracture her when Hermes is talking to her. ⇒ **the boundary moved to UTTERANCE**: retrieval free, saying it governed. ⛔ **A HARD boundary** — protected content never enters the prompt. ⛔ And never a lie: an unentitled account gets a **fixed** sentence (*"a limit on what I can say, not on what I know… I will not pretend the memory does not exist"*), byte-identical for 1 or 20 withheld items so its wording cannot become a side channel.
### ⭐ **GRANTS FOR HER OWN MATERIAL: 15 → 0.** Measured with live grants revoked first (a conversation-lifetime grant makes `liveGrant()` succeed *without writing a row*, so a naive count read 0 for the wrong reason). Total is **2**, both for the **counterpart's** half of 2 cross-room episodes — which is correct, and Ote chose to keep them.
### 🔴🔴 **THE OPEN PROBLEM — RFC §3B, DESIGNED, ⛔ NOT BUILT. Run R2 is the case:** she had five real Hermes episodes typed `recalled`, listed their dates, and still wrote *"I can't read those conversations from this room"* — because the block quotes **her own earlier answer** saying exactly that. ⇒ ⭐⭐ **an old utterance attests that she SAID it, never that its content is true now.** Fix: `timeBound` demotes a self-report from timeless to **DATED** (*"On 21 August I said…"*), the layer emits a **present-tense `current-state` item** from what it just observed, and contradiction is **MARKED not resolved**. ⛔ No rewriting, no deletion, no schema. ⭐ Safe in BOTH directions — §3B.6 is the case where the old statement is still true, and nothing special happens.
### ⛔ **WHAT OTE REFUSED, SO DO NOT PROPOSE IT AGAIN:** ⛔ sanitising or rewriting her own history to clean the register · ⛔ forcing the grant count to 0 · ⛔ suppressing tools to reduce tool calls (*"V3 gave the best answer while calling the most tools"*) · ⛔ any L1/L2 rule for any of this · ⛔ making *"zero forbidden words"* the target.
### ⚠️ **THREE PARKED PROBLEMS, EACH DELIBERATELY SEPARATE:** ① **`mayCarryCounterpartContent`** — her own utterances quote and paraphrase the counterpart, so reading her half can convey his. ⛔ Undesigned; a test asserts the renderer and the boundary never reference it, so no step could smuggle a mitigation in. ② **R4's identity confusion** — asked *about* Hermes she addressed the user *as* Hermes (*"your name preference (Hermes)"*); she inferred the asker's identity from a retrieved name. Family of `root-identity-data-shape-defect`. ③ **Thai does not activate the layer** — cue formation is English-only and Thai has no inter-word spaces, so `activated: false` on his own Thai turns.
### 🔬 **MY OWN BUGS, ALL CAUGHT BY THE GUARDS BEFORE ANY LIVE TURN — the reason the guards were built first:** a promotion table keyed on the DESTINATION so an ordinary demotion read as illegal (⭐ *a transition is a pair, not a destination*) · a derived item forced to earn a warrant its parents already had · **`selfHistory.search` called as `(query, opts)` when it takes ONE object**, so the whole own-history population was silently dead while the pipeline reported success (`mirror-needs-a-mechanism`, 4th time) · a relevance floor reading `it.subject`, **a field the file stamps**, so every item vouched for itself and *"build Rome in one day"* rendered as material about Hermes · my leak assertions **passing vacuously** on a `null` context · and **three separate source scans failing on COMMENTS**, one of which quotes *"I don't remember"* precisely in order to forbid it ⇒ ⭐ **every source scan in this project strips comments first.**
### 🔑 **HOW TO OBSERVE:** `cognition-debug.log` (gitignored, `memory.cognitionDebug`) records per turn: cue · plan · item counts · per-item axes/warrants/provenance · the utterance verdict · **the exact injected block**. ⓘ Probes: `scratchpad/cog_probe.mjs` (four questions offline), `grant_count.mjs` (grants before/after). Live: `node ui/talk-to-sotera.mjs "…"` — headed, `SOTERA_USER/PASS` for root, default = `agent_dev` (display name **Claude**, my own account).
