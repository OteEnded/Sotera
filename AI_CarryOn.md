...
### ⏭⏭ **RESUME HERE — 2026-08-24, 15:19. ⛔⛔ STOP. PHASE C IS CLOSED BY OTE AND HE IS CHOOSING THE NEXT JOB HIMSELF.** *"Do not start another phase, investigation, redesign, cleanup, or architecture work on your own… After that, wait for me."* ⚠️ The research → external-evidence → decision-comparison workflow is a **CANDIDATE, NOT A DECISION**. Suite **35/35**. Sotera `e358398`, `Reference` clean. Corpus clean, 0 harness leftovers, **34 decision rows across two rooms, 17 of 17 resolving**.
### 📄 **READ THESE FIRST** (⛔ `Reference/` is its own git repo): **1. `docs/DESIGN_SOTERA_SKILL_CONTRACT.md`** — what a Skill is, §10 = the three runs that built the authoring rules. **2. `docs/MEASUREMENT_SOTERA_OWNERSHIP_FALSIFIER.md`** — Parts I–III: the falsifier, the 481/482 finding, W1 shipped, §17 the closing ledger. **3. `docs/DESIGN_SOTERA_WORKING_STATE.md`** §10 — working memory vs todo. **4. `docs/RESEARCH_OKF_LLMWIKI_UNDERSTORY.md`** §9 — his three open items, answered.
### ⭐⭐⭐ **THE MODE HAS CHANGED, AND THIS IS THE MOST IMPORTANT LINE HERE.** From *"is the cognition architecture correct?"* to **"is Sotera actually useful?"** ⛔ He has said, repeatedly and in his own words, that he does not want *"build infrastructure → investigate infrastructure → find edge case → build another layer"*. ⇒ **Start from the JOB. Never from a capability we could build.**
### ✅ **THE TWO DEMONSTRATED REAL JOBS — preserve these.** **`skill.prior-decision`** — *"have we been here before?"* → looks up ONE decision, reports status + source. **`skill.doc-reconcile`** — a SUPPLIED DOCUMENT → what is already decided / conflicts / new / unresolved, with a source on both sides. ⭐ Both installed via `authorSkill`, both declaring `metadata.required-artefacts`, both trigger-first in their descriptions.
### ✅ **`list_decisions`** — native to the chat route, **read-only**, one SELECT, `entity='project-decision'` and nothing else, caller's room only, returns the record **plus its source reference and verbatim quote**, projection spelled field-by-field so a new column cannot ride along, and an empty result says *"none was written down, not that the matter is undecided."* ⛔ It changed **nothing** about retrieval.
### ✅ **PROVENANCE IS A HARD PROPERTY.** `test/maintenance/seed-decisions.mjs` — 17 decisions, each **real path + real commit + verbatim quote**, verified with `git show` BEFORE any write, and it **refuses the whole batch** on one bad quote. ⭐ `--verify` re-resolves every stored row independently of her. ⚠️ **TWO REPOS** (`Reference` and `Sotera`) because a document is the source for *what did we decide* and the CODE for *is it implemented*. ⛔ Never seed anything that cannot be verified; ⛔ never weaken this to make a run pass.
### ⛔⛔ **THE LESSON THAT COST THE MOST: INSERTING INTO A MEMORY TABLE IS NOT THE SAME AS REMEMBERING.** The first seeding looked perfect and the job still said *"no prior decision found"* — raw SQL bypasses the write path, so every row had `embedding IS NULL`. ⚠️ And my FIRST fix was wrong: I edited the Skill to tell her to enumerate instead of search, which is coaching around a defect rather than fixing it.
### ✅ **PLATFORM · TURN COMPLETION stays.** One ephemeral `last_round` nudge (asking for the answer **and** for what she did and did not finish), an **unconditional** `rounds_exhausted` status, and an error stamp when a short reply follows a full budget. ⛔ I withdrew my own diagnosis here: I "raised the cap" by writing `chat.toolsMaxCalls`, a key **nothing reads** (it is `chat.tools.maxCalls`), so the run I credited to headroom succeeded on variance. **The warning is the fix.**
### ✅ **SKILL ARTEFACTS = FORM VERIFICATION, NOT TRUTH VERIFICATION.** ⛔⛔ Keep that distinction: the `Checked:` section appeared and contained **two fabricated citations** (*"the web search confirms…"* with `search_web` never called; *"verified at 2a739f3c"*, no such conversation). ⇒ ⭐ **she fabricates PROVENANCE, not content.** Presence proves the shape and nothing more.
### 🔧 **THE INSTRUMENTS.** `pipeline/verify-answer.mjs` — resolves an answer's references against git AND against stored records, checks claimed-vs-called tools, quotations, statuses, and **classifies by layer**. `pipeline/router-reliability.mjs` — two phrasings per skill, all four rates, self-cleaning. `maintenance/memory-lint.mjs` + `checks/memory-lint-check.mjs`. `maintenance/seed-decisions.mjs`. `pipeline/ask-sotera.mjs` — `--as`, `--cid`, **`--skill` to BIND**, and a turn of the form **`prompt@path`** to feed a document.
### ⛔⛔ **THE VERIFIER HAD TO BE BEATEN INTO HONESTY FIVE TIMES — read this before trusting any number it prints.** False positives I caught and fixed BEFORE reporting them as her failures: naming a tool ≠ claiming to have used it · **a truncation is not a fabrication** · her own bold markup captured as a quotation · a status read off the SECOND mention while the cited one was right · every Sotera-repo reference reported as *"path does not exist"*. ⭐ The quote check is now a **WARN** because its haystack cannot see tool results. **A check needs to prove what it claims to prove.**
### ⭐ **THE RELIABILITY NUMBERS, before → after.** activation **56% → 69%** · wrong-skill **9% → 0%** · no-skill 34% → 31% · artefacts **1-of-2 nudged → 11/11, 7/7 recovered** · `prior-decision` natural **2/8 → 3/4** (phrasing effect gone). ⚠️ n=4 per cell — directional, not established. ⛔ **Do NOT chase the residual `doc-reconcile` variance unless he reopens it.**
### ⛔⛔ **FROZEN, AND HE RESTATED IT AT CLOSE:** relevance floor · **Defect A** · working-memory architecture · document ingestion · **O1** / the flattened container · **D3 · D4 · P5** · L4 beyond B0 · identifier projection · `about0` · `mayCarryCounterpartContent` · stop-lists · `windowRadius` · Memory V3 / relations-as-objects · OKF export. ⏸ And two Skill-contract decisions await him, **not tasks**: **S1** (`allowed_tools` honoured when bound, ignored when triggered) and **K1** (`skill` is the only composer part whose `foundational × task` the platform's own table prohibits).
### ⚠️ **STANDING WARNINGS.** ⛔ **ROOT IS HIS ACCOUNT** — probes refuse `ote`; ⓘ 18 of his 23 conversations are **archived** (soft, reversible) and her retrieval ignores `archived_at`, so his list is tidy and her memory intact. ⚠️ `hermes` + `hermes_alias` hold `sotera_memory` — **HIS grants**, audited, ⛔ do not revoke. ⚠️ `agent_dev.display_name` is **`Claude`** — my residue, a live confound in identity tests. ⚠️ Restore the entitlement grant before any measurement needing cross-room reach and **revoke it after**. ⚠️ Server is `npm start` from `Backend/` — ⛔ **never `--watch`**; `config.json` is read at boot and several changes this phase had **no flag**, so a code change needs an explicit restart.
### 🔬 **MY REPEATED FAILURE MODES — read before trusting anything I wrote.** ⛔⛔ **A rate is only as good as its screen, and I am now at EIGHT bad ones** — the newest: a canon violation reported from **half the liveness predicate** (`expired_at` only, when it is `invalid_at AND expired_at`) and a Thai-blind census. ⛔ **An allowlist drops what it was not told about — thirteen instances**, and this phase added the *leaking* direction (a rule that reports **0** when its table is missing). ⛔ Verify a claim against **its own** source: I flagged one of her dates by comparing a *message* date to a *conversation* date, and she was right. ⛔ Scripted edits destroy escapes — `\n` inside a python heredoc becomes a real newline in JS. ⛔ A `const` used above its declaration **parses fine** and throws at runtime.
