...
### ⏭⏭ **RESUME HERE — 2026-08-26, ~00:02. ⏭⏭⏭ NEXT: ① THE HANDLE-SCHEME DEFECT → ② the history purge → ③ the Gen-3 read → ④ the web-search salience probe.** ⚠️ **① is a live blocker:** in a CLEAN conversation she found the source, then *"Every retrieval call consistently returned the handle as `7198c1b0de`, yet the system keeps rejecting it as too short."* ✅ Established: `handleFor(cid)` DOES produce `7198c1b0de`, and the 8-char form is correctly refused. ⛔ NOT established: why the 10-char form failed at the **disclosure** boundary. ⓘ Suspicion, unverified: `conversation-retrieval.handleFor()` and the disclosure host's `locateConversation` may not share a scheme — one tool emits a handle another will not take. ⚠️ **RESTART SOTERA EXPLICITLY and verify PID/start time before any live claim** — and ⛔ the restart script only scans `Backend/app`, so an edit under `Backend/database` or `Backend/providers` will NOT move `newestEdit`.
