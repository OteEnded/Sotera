...
### ⏭⏭ **RESUME HERE — 2026-08-26, 10:15. SUITE 39/39. `d1b2051` · MIGRATION 030 APPLIED.** ⏸⏸ **STOP: THE THREE REMAINING DECISIONS ARE OTE'S** — **A** the modality vocabulary (minimum set, **act-vs-term**, and the `figurative` storage policy) · **B** whether a contradicted memory stays retrievable with its contradiction attached or leaves normal recall · **C** whether the confidence scale changes. ⭐ Options, costs and the **Rome case worked through each one**: `Reference/docs/DECISIONS_SOTERA_MODALITY_CONTRADICTION_CONFIDENCE.md`. ✅ **WHAT SHIPPED** (everything buildable without a values call): `memory-lineage.js` (`mechanismOf` — *“is this an extraction?”*, all 30 source tags · `evidence.derivedFrom` — the **derivation** axis, additive, pointers only) · `memory-retrieval-trace.js` (⭐⭐ **the record that did not exist** — the route dropped the ids in a `.map()`) · `memory-contradiction.js` (candidates from **presence**, ⛔ never similarity, fails closed) · **030** `contradicted_by_message_id` + `contradicted_at` + `store.markContradicted`. ⚠️ **SHE IS STILL SERVING THE PRE-`d1b2051` BUILD — the trace wiring needs a RESTART, and that is HIS call.** ⛔ UNTOUCHED AND STAYING SO: the three lineage rows · the four Rome rows (`7d383ce3` is **still live and unmarked**, asserted on every run) · Gen-3 texts · `56425175` · the purge gate · OteRM (**observation only**).
