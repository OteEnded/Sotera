...
### ⏭⏭ **RESUME HERE — 2026-08-26, 09:30. SUITE 38/38.** ⏭⏭⏭ **NEXT: PHASE 5 IMPLEMENTATION, in this order** — ① `provenance` on **extraction** · ② `evidence` **lineage** on synthesis · ③ `contradicted_by` written **from the retrieval trace**. Design: `Reference/docs/DESIGN_SOTERA_CORRECTION_PROPAGATION.md`. ⛔ **THREE THINGS NEED OTE FIRST:** the **modality vocabulary** (asserted · aspirational · figurative · quoted · hypothetical — and whether `figurative` blocks durable storage outright), whether a **contradicted memory stays retrievable** with its contradiction attached, and whether the **confidence scale** changes on an existing 92-row store. ⛔ UNTOUCHED AND STAYING SO: the three lineage rows · the four Rome rows · Gen-3 texts · `56425175` · the purge gate · OteRM (**observation only**).
