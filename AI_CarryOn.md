...
### ⏭⏭ **RESUME HERE — 2026-08-21 ~16:00. THE MEMORY COGNITION LAYER IS LIVE AND THE OWNERSHIP MODEL IS THE OPEN DECISION.** Suite **29/29 · 942 assertions** · schema through **020** (⛔ 021 is *sketched only*). ⚠️ Server runs `npm start` — ⛔ **NEVER `node --watch`**, Ote's instruction after it caused three false alarms in one day (a lost fire-and-forget audit write, a `boot-check` hit mid-reload, and — worst — `config.json` is read once at boot so watch never reloaded a flag while `/health` returned 200).
...
### ⛔⛔ **THE OPEN DECISION — RFC §3A, `Reference/docs/RFC_MEMORY_COGNITION_LAYER.md` (workspace root, NOT under the project).** Ote: *"Sotera should be Sotera's memory… If Sotera remembers something from a conversation in another room, she should be able to recall it because it is her memory, not because the current account has been granted access to that room."* ⭐⭐ **THE FINDING: OWNERSHIP IS NOT REPRESENTED ANYWHERE** — it is inferred from the room, which is why *"my memory stores are scoped to this room"* is a **true report**, not a misunderstanding to fix with a prompt. Two domains: **Sotera memory** (hers, one memory, not fractured by room) vs **account memory** (the person's, authorization required). ⭐ Proposal: `mst_users.memory_access_scope` (`none|sotera_memory`) + `can(user,'access_sotera_memory')`, root as the **granting authority** rather than root-ness being the mechanism. ⏸ **ONE QUESTION BLOCKS migration 021:** the capability is per-ACCOUNT but Sotera is not an account — so granting it to `ote` gives Ote's sessions a whole Sotera while `hermes` stays fractured. That may be right, or the scope may belong on the **persona**. ⛔ Ote's call.
