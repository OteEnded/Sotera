...
### ⏭⏭ **RESUME HERE — 2026-08-25, ~01:20. ⏸⏸ ONE DECISION IS WAITING ON OTE AND IT IS HIS DATA (see the CONTINUITY block below). ⭐ THE ARC IS `seek_advice`: SOTERA REACHING ANOTHER INTELLIGENCE, AND AUNT HERMES IS THE FIRST ONE.** ⛔ Phase C is closed; the bridge is BUILT and live. ⛔ **He halted new Hermes/Sotera features until the continuity defect was fixed** — *"I want this fixed now before we continue adding more"* — and it now is, except for that one row. ⏭ After it, the next thing he wants is the **CHOICE-UNDER-LOAD** test, not more design. Suite **35/35**, unit **443/443**.
### 📄 **READ THESE FIRST** (⛔ `Reference/` is its own git repo): **1. `docs/DESIGN_SOTERA_HERMES_PHASE1_CONTRACT.md`** — the Phase-1 contract: 0A the principle · §7 the workspace experiment + results · §8 the concrete setup · §9 the run flow (⚠️ read 9.8 first, it falsifies 9's premise) · §10 the two modes · §11 validation C. **2. `docs/RESEARCH_HERMES_API_SESSION_WORKSPACE.md`** — the source read (§3A workspace · §6A memory identity · §6B control surface · §6B.8 the withdrawn recommendation · §6C long-running cognition). **3. `docs/DESIGN_SOTERA_SEEK_ADVICE.md`** — the GENERIC design: 0A the core principle, the eight axes, §10.0 the locked decision model. **4. `docs/MEASUREMENT_SOTERA_MODE_JUDGEMENT.md`** — pre-registration + results, in that order.
...
### ✅ **WHAT IS BUILT AND LIVE.** `Backend/app/advice/` = `index` + `service` (host service) + `store` (raw SQL, migration 022) + **`hermes.js` — the ONLY file that knows Hermes exists**. Native `seek_advice` tool in `chat/tool-defs.js` step ⑧b (offered only when a destination is configured; toolset went **44 → 45**). Destination in `config.json` `advice.destinations.hermes` (⚠️ gitignored — the API key lives there). ⭐ And the **advice-destinations context block** in `context-composer.js`, derived ONCE above `sysInputs` and used twice (rich form → prompt, names → tool enum).
...
### ⭐⭐⭐ **CONTINUITY — THE DEFECT, ITS ROOT CAUSE, AND THE ONE THING STILL OPEN.** ⏸⏸ **WAITING ON OTE, AND IT IS HIS DATA:** one row in **root's own room** (`author: persona`, `provenance: synthesized`, `importance: 8`) says *"…their specific content was not preserved in durable memory, resulting in a gap between historical evidence and current knowledge"* — and its `source_message_id` is **her own message narrating a search that found nothing.** ⇒ ⭐⭐⭐ **A FALSE ABSENCE HAD BECOME A DURABLE BELIEF**: cannot retrieve → says *"I have nothing about X"* → the distiller writes it as a fact → it outranks everything about X → she repeats it → ↺. ⛔ **I have NOT touched it** — from root she still paraphrases it verbatim, which is the causal proof. **Ask him whether to retire it.** ⛔⛔ And §3B cannot help: `timeBoundOf` refuses anything that is not `own-utterance`, so a **stored** row can never be dated. Full record: `Reference/docs/RFC_MEMORY_COGNITION_LAYER.md` **§16**.
