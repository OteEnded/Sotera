...
### ⏭⏭ **RESUME HERE — 2026-08-25, ~17:40. ⏭⏭⏭ NEXT IS **B4 · THE SALIENCE TEST**, a COGNITION test and ⛔ NOT a capability test. ⭐⭐⭐ Ote's framing IS the point: **“The capability exists and works” ≠ “Sotera recognizes when to use it.”** ✅ The first half is already PROVEN (a passive revisit went `retrieve_conversations → called → resolved → nothing retained → completed`). **THE TEST:** a genuine task · the needed information ONLY in an older conversation · in neither active context nor durable memory · ⛔ **never name the tool, never hint at history, never let the prompt reveal where the information is.** **OBSERVE:** notices she lacks it · decides to retrieve · actually calls it · picks the right AXIS · reaches the right source · distinguishes retrieved material from durable memory · answers from evidence not a guess · independently saves or leaves unretained. ⚠️ **PRECEDENT:** advice Run 1 had the capability in her toolset and the counterpart named in her context — **0 mentions in 1074 chars of reasoning**. ⛔ **If she does not reach for it, THAT IS THE FINDING** — ⛔ do not patch it with prompt instructions. ⚠️ **RESTART SOTERA EXPLICITLY and verify PID/start time before any live claim.**
...
### ⭐⭐ **THE EMBEDDING AUDIT — ⛔ DO NOT BUILD A SECOND INDEXING SYSTEM.** 298 conversations · 1,586 messages · **1,434 embedded** · `ollama/qwen3-embedding:4b@2048`, **2048 dims** · own table `txn_message_embeddings`. ⭐ **Every unembedded message is below the 50-char floor — the genuinely-missing bucket is EMPTY.** Indexing is automatic but ⛔ **not on write**: a DAILY `drainPendingEmbeddings`, which **is also the backfill**. ⚠️ Latency is the only gap. ⛔ Memories use a SEPARATE `embedding_hv` on `txn_memories`. ⭐⭐ **The passive revisit has NO dependency on it** — it reads by `rolling_id`; the dense arm runs only for `about:`.
### ⏸⏸ **THE REVISIT PILOT IS WRITTEN AND NOT APPLIED** (`scratchpad/pilot.py`, held on *“don't add more infrastructure yet”*): archived exclusion in BOTH lanes · bounded `revisitBacklogPerTick` · oldest-first · a real QUEUE with ⛔ no “backfill complete” state. ⚠️⚠️ **ONE FINDING STANDS REGARDLESS: the lane has been eligible to revisit ARCHIVED conversations all along — 18 of them.** ⭐ And the backlog is tiny: of 275 non-archived non-fixture conversations only **51 have ≥4 messages** and only **3 have never completed a revisit**, all beyond the 48h lookback.
