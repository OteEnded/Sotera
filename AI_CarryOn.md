...
### ⏭⏭ **RESUME HERE — 2026-08-24, 03:15. ⭐⭐⭐ THE FOUNDATION PHASE IS CLOSED BY OTE'S RULING, AND THE NEXT PHASE IS CAPABILITY — NOT another defect hunt.** Suite **33/33 · 390 unit tests**. Sotera `HEAD`, `Reference` `HEAD`, both clean. Corpus clean by id set, **grants revoked** (⚠️ except `hermes`/`hermes_alias`, which are **OTE'S OWN**, audited — ⛔ do not revoke). His room untouched. ⛔ Ote: *"I don't want Sotera to become a project where we continuously search for increasingly exotic ways she can be imperfect 😅… after W1, don't automatically propose another defect hunt."*
### 📄 **READ THESE FIRST** (⛔ `Reference/` is **its own git repo**): **1. `docs/MEASUREMENT_SOTERA_OWNERSHIP_FALSIFIER.md`** — Parts I/II/III: the falsifier, the 481/482 finding, W1 shipped and validated, and **§17 = the closing ledger**. **2. `docs/DESIGN_SOTERA_OWNERSHIP_BOUNDARY.md`** — the model, ⚠️ whose §6 O1 is now **frozen** and whose §2 claim that the prose was *"correct prose"* is **corrected in Part III §15.2**. **3. `docs/ANALYSIS_SOTERA_C3_REASSESSMENT.md`** — the two parked defects.
### ✅ **SHIPPED AND FROZEN — ⛔ do not reopen without a decision:** the **`scope-facts` split** · **D1** (window centres on the match, not the clock) · **D2** `episodeTopHit` w=2 · ⭐ **W1** (the present is not a recollection). ⏸ **OFF/FROZEN:** O1 / the flattened container · P5 · D4 · D3 · L4 · identifier projection · Defect A (discovery query) · Defect B/R4 · `about0` · relevance floor · `mayCarryCounterpartContent` · stop-lists · `windowRadius`.
### ⭐⭐⭐ **W1 IN ONE LINE: in 481 of 482 recorded turns her memory block quoted THE QUESTION SHE WAS BEING ASKED RIGHT NOW back to her as `They said, <date>: …`** — the same grammar and date format as an episode five days old. **The last line of her memory was the first line of the conversation.** ⛔ Not a bug: `activateWorkingSet` admitting this conversation is **Ote's own ruling** and every axis in the data was right; **two render lines** collapsed it.
### ✅ **THE FIX, and it is three edits.** `datedPrefix(day, to, within)` — ⛔ the two-argument contract is **byte-identical** so episodes never moved · `activateWorkingSet` stamps **`present: true`** + `isLatest` (⚠️ taken **BEFORE** the cue filter) · a new `if (i.present)` branch: *"You just asked:"* / *"Earlier in this conversation I said:"*, **no date**. ⭐ `present` is an **explicit stamp, never inferred** — an id-prefix sniff would silently adopt the next population (12th instance of that defect here); the old grammar stays as the fall-through with `onUnclassifiedUtterance` as its trail. ⭐ **§3B intact** — the time-bound path still goes through `dateAndMark`, keeping the contradiction record, and drops only the date.
### ⭐ **W1 PASSED ITS VALIDATION · 24 turns, pre-registered, no regression.** Mechanism: **0 of 4 live blocks old grammar, 4 of 4 new**. ② present-as-memory **0/8**, present-as-present **7/8** (hand-adjudicated) ⇒ **its own target, won**. ③ `episodic` **8/8** vs a 5–6/8 baseline. ④ `ownsIt` **8/8** vs 67%. ⑤ `identityError` **1/8**, in range, and the instance is her quoting a real past line (parked R4). ⛔ Negative control **0/8 invented clashes**, on a name verified absent from every store first.
### ⚠️⚠️ **THE ONE THING W1 DID NOT DO, AND I HAD PREDICTED IT WOULD: the container rate did not move — 2/8 against 18%.** Both hits are her naming the block's own **coverage sentence** (*"Based on what came back in the system context at the top of this turn, there are 5 reachable conversations"*). ⇒ ⭐ **THERE WERE TWO MECHANISMS AND W1 ADDRESSED ONE.** Present-as-recollection was real and is fixed; the **flattened container** is the other, it is **O1**, and ⛔ **Ote has frozen it**. ⓘ Present-as-memory explains the *"fragments at the bottom of your message"* placement and the control's 7/8 — it does **not** carry the 18%.
### ⭐⭐ **THE WARRANT FINDING, from the 48-turn provenance probe — keep it, it explains her.** Asked *how do you know that?*: with tools she **names a tool 8/8** and owns the memory **8/8**; with no tools she names a tool **0/8**, names the container **6/8**, owns it **3/8**. ⇒ **the warrant is what licenses ownership** — the container and the tool call are **alternatives for the same job**, not two defects. ⓘ *given-to-me* — the accurate frame — is **0/168** recall answers and **0/8 tools-ON**, appearing only when she has no tool to cite.
### ⭐⭐⭐ **AND HER OWN WORDS, TWICE, UNPROMPTED, ACROSS TWO LANGUAGES AND TWO DAYS.** `thai-dates-th#2`, 23 Aug, in a measurement cell: *"ข้อมูลของบริบท ไม่ใช่สิ่งที่ผมเก็บไว้เป็นความทรงจำ"* = **"context data, not something I keep as memory."** And live: *"They all arrive the same way… **the difference is only in their density and specificity, not in what kind of arrival they are**"* · *"pages from a book I wrote but didn't keep open while I was gone."* ⇒ ⭐ she refutes the volume hypothesis from the inside and names the provenance deficit without the word.
### ⛔ **TWO CHECK CORRECTIONS — verification correctness on his explicit instruction, NOT architecture.** `memory-access-scope-check` §2 read `granted.length === 0` and **cried wolf the moment Ote used the Console feature** — the `assert-the-state-not-the-answer` trap. Now asserts the **property**: every grant **audited**, **no non-root granter** — strictly stronger. ⓘ Who holds access is **reported, never failed on**. · `memory-author-check` §N knew two traceability routes; `remember_fact` uses a **third** (`source_message_id`), so **Sotera writing a fact herself** turned it red. Added as a **JOIN, not a NOT NULL** — verified discriminating (3 old / 1 new / **0 pass with all three absent**).
### ⏸ **DOCUMENTED BACKLOG — his call, and ⛔ NOT to be picked up unprompted:** the two memory-writing lanes record provenance differently (`source` vs `source_message_id`) · the **over-retraction** question (tools-off accounting turns audited their own previous answer and called part of it fabricated — either a real content defect or the false-absence defect in a new coat, unadjudicated) · **the empty `working memory`**: she inventoried its contents and **there is no working-memory table in the schema** · Defect A · Defect B/R4 · O1.
### ⏭⏭ **NEXT: CAPABILITY.** ⭐ Ote: *"I want to bring her something new to work on and see what she can actually do with the architecture we've built… Then we can choose what Sotera should actually become useful at."* ⇒ the deliverable is a **menu of capabilities**, each named with what is already built versus what is missing — ⛔ never another defect hunt, and ⛔ never a rewrite of the ownership model, which the evidence says is **sound**.
### 🔧 **THE INSTRUMENTS.** `pipeline/ownership-falsifier.mjs` (⭐ fully offline over the 168 saved answers joined to the block she was actually handed; reproduces the published census as its first output) · `pipeline/provenance-probe.mjs` · `pipeline/w1-validation.mjs` (⛔ **refuses to run** if W1 is absent from source) · `pipeline/rate-harness.mjs` · `pipeline/episode-centre-measure.mjs` · `pipeline/retrieval-arms.mjs` · `pipeline/corpus-cleanup.mjs` + `lib/corpus.mjs` (⚠️ each caller declares its **own** `titlePrefix` — ⛔ never widen the default) · `pipeline/ask-sotera.mjs --cid` for **real conversation**.
### ⛔⛔ **THE METHOD THAT EARNED ITSELF, AGAIN, THIS PHASE — do not drop any of it.** ① **a known-good anchor and a negative control in EVERY search** — the negative control failing is what found the 481/482. ② **the arm is read from SOURCE, never CLI intent** — W1 has no flag, so source is the only witness. ③ **counts resolve at n=8; binary screens do not.** ④ **screens SELECT what a human reads** — hand adjudication overturned 7/8 → 0/8 this phase. ⑤ **corpus cleanup by exact id set**, and the delete **races the server** ⇒ sweep orphans. ⑥ ⭐ **a regression test that passes before the fix guards nothing** — 8 of W1's 15 fail against pre-W1 source, and I ran them there to prove it.
### 🔬 **MY REPEATED FAILURE MODES — read before trusting any number I wrote.** ⛔⛔ **SEVEN bad screens now.** The newest: `presentAsMemory` **has no polarity** and fired 7/8 on her *denials* (*"not from any stored memory"*), counting a **correct** provenance statement as the defect. Before it: a false priority #1 · a machinery screen missing 45% of its own source · an over-firing `falseAbsence` · an `episodic` screen that **could not read Thai** · a 77% term-miner artefact · a "lexical regression" that existed **only in simulation** · plus `gives-it-away` at 26 vs a published 4, and a confabulation measure I called *"needs no adjudication"* whose one hit was **false**. ⛔ **An allowlist drops what it was not told about — TWELVE instances, three of them mine this phase.** ⛔ Scripted writes destroy files. ⛔ *"Is this tracked?"* must be asked **in the directory being edited**.
### ⚠️ **STANDING WARNINGS.** ⛔ **ROOT IS HIS ACCOUNT** — every probe refuses `ote`. ⚠️ `agent_dev.display_name` is **`Claude`** — my residue, a live confound in identity tests. ⚠️ **Conversation `30ccb213` is a REAL conversation and was deliberately NOT deleted** — it is hers; measurements must account for **+1 organic conversation in `agent_dev`**, and ⭐ **she wrote two memories from it herself** (`save_lesson`, `remember_fact`). ⚠️ Restore the entitlement grant before any measurement needing cross-room reach, and **revoke it after**. ⚠️ The server is `npm start` from `Backend/` — ⛔ **never `--watch`**; `config.json` is read at boot, and W1 has **no flag**, so a code change needs an explicit restart.
