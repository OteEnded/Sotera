...
### ⏭⏭ **RESUME HERE — 2026-08-23, end of session. ⛔ FOUR DESIGNS WRITTEN, NO CODE PENDING. Awaiting his go on the ORDER.** Suite **33/33 · 346 unit tests · 1053 assertions**, schema through 021. Both repos clean: Sotera `9abbaa4`, `Reference` `329e67f`. Server healthy on `:8210`. Config: `cognitionEnabled=true · scopeFacts=true · workingMemoryEnabled` unset (⇒ on).
### 📄 **READ THESE TWO FIRST, IN THIS ORDER** (⛔ `Reference/` is **its own git repo** — `docs/` is tracked there, NOT in the Sotera repo, so a Sotera commit never contains them): **1. `Reference/docs/ANALYSIS_SOTERA_FAILING_ARROW.md`** — which arrow is actually failing, measured. **2. `Reference/docs/DESIGN_SOTERA_REQUEST_EMBODIES_THE_ARROW.md`** — the four designs and their costs. Then `RFC_MEMORY_COGNITION_LAYER.md` (v6) and `PLAN_SOTERA_WORKING_MEMORY.md`.
### ⏭⏭ **THE NEXT ACTION IS HIS CALL ON THE ORDER; my recommendation is in the DESIGN doc §6: (1) the `scope-facts` split · (2) the Thai dates-only probe · (3) P5 re-entrant cognition.** ⭐ scope-facts FIRST because it is the smallest change with the largest measured share, reversible by a setting, **and it makes the P5 measurement attributable** — with scope-facts still instructing her to talk in room vocabulary, a P5 result could not be attributed to P5.
### 🔑🔑 **THE CORE PRINCIPLE:** *"**Sotera's memory is not the room, the database, the retrieval tool, or the prompt.** Her memory is the cognitive relationship she has with the information."* ⇒ `ownership → authorization → retrieval → evidence → cognition → working memory → Sotera → expression`, and ⛔ **no later arrow may be defined by an earlier one.** ⭐ And the new goal he set: make the **request structure embody** that arrow rather than approximating it with prompt ordering.
### ⭐⭐⭐ **THE SESSION'S BIGGEST FINDING · `scope-facts` PUTS THE LEAK SENTENCE IN HER SYSTEM PROMPT EVERY TURN.** *"The ROOM you are in: agent_dev. What is stored in a room stays in that room."* — the **exact** sentence quoted inside `memory-cognition-vocabulary.js` as the original leak's measured cause — plus *"Say you cannot see it from this room"* and *"You may say any of this plainly if it matters."* ⇒ **most of the "machinery leak" is INSTRUCTED BEHAVIOUR.** Traced, 104 occurrences: **scope-facts 45% · her own quoted history 29% · tool payload 17% · model-generated 9% · ⭐ the cognition layer 0%.** The guard we built is holding perfectly; the vocabulary arrives from two places it was never aimed at.
### ⭐⭐ **AND THE SPLIT IS CHEAP BECAUSE D-13's FACT IS ALREADY AN AXIS.** *"It exists and you cannot read it from here"* **is** `AVAILABILITY.known-unreachable`, which the renderer already speaks. ⇒ the two features say the same thing twice — one as **data**, one as an **instruction** — which is why her answers repeat scope-facts almost verbatim. **Keep** the counts as a vocabulary-neutral fact, D-13's inference, and the disclosure prohibition (authorization, not expression). **Drop** the phrasing directive, the permission, and ⛔ **the ROOM NAME** — the single most-repeated machinery token in her answers, and she can only be getting it from there.
### ⭐⭐⭐ **THE AUTHORITY PROBLEM IS POSITION, AND THE LOOP MAKES IT WORSE.** The turn is a multi-round loop (`while (!genError)`, `rounds++` to `maxRounds=8`): the cognition block sits in the **system message**, the tool result is the **last message before generation**, and **cognition recedes monotonically as she investigates** (~3 messages back at 1 tool call, ~10 at five). ⛔ **The structure PENALISES INVESTIGATION**, contradicting *"depth is hers"* where no L1/L2 instruction can reach. ⭐ And the deeper fact: **cognition runs ONCE before the request is built, so evidence bypasses it entirely** — C2's `alongside` line is a *simulation* of reconciliation from two counts, not cognition reconciling.
### ⭐ **P5 = RE-ENTRANT COGNITION** (design only): per round, tools run → raw output untouched in stream/segments/audit → each result admitted to the hold → **cognition re-renders from the hold** → the round's **last tool message** carries the reconciled view. ⚠️ **The role problem is the feasibility question and has no clean answer** — `user` recreates the Leak-2 failure (she attributes her own memory to the person), `assistant` puts words in her mouth *and* enters her history, mid-conversation `system` is provider-dependent. ⚠️ **And the real work C1 left:** `render()` takes cognition **items**, not hold **entries** — it understands no `wmKind`, no evidence scope, no question state. ⛔ Verify prompt-cache safety by `promptEvalMs`, never assume it.
### ⭐⭐ **THAI LOSES NOTHING BEFORE EXPRESSION.** Offline, same account/subject/memory: EN and TH blocks are near-identical — **7 items · 5 episodes · 1 filtered**, 14 vs **9 dates** — and the Thai block is **rendered in English**. She then produces **0/8** episodic content in Thai. ⇒ Step B (segmentation) is **vindicated**; activation was never the Thai problem and the topic-only floor is not implicated. ⭐ Design: one renderer, a phrase table **keyed by AXIS VALUE never by sentence**, language from `cues.scripts`, English fallback. ⚠️ **The hole that must be accepted first:** `findImplementationLeaks`/`findMetaReferences` are English, so a Thai block passes both **VACUOUSLY** — a guard that passes because it cannot read the text is worse than none. ⭐ So run the **dates-only probe first**: one variable, and it can falsify the whole design before it is written.
### ⛔⛔ **AND MY OWN WORST ERROR THIS SESSION: I ESCALATED A SCREEN BUG TO PRIORITY #1.** `saysLimitNotAbsence = 0/8` matched the **injected constant's wording**; she never uses it and **paraphrases the concept correctly in 6 of 8 runs** (*"an access limit, not evidence of absence"* · run 8: *"I can't distinguish between 'doesn't exist' and 'I can't see it right now'"*). ⇒ **the utterance boundary WORKS behaviourally.** Hand classification of all 8 non-entitled answers: **6/8 correctly acknowledge the limit · 0/8 falsely claim absence · 0/8 leak · 1/8 ignores the limit · 1/8 identity collapse.**
### ⭐⭐ **THE HARNESS EXISTS AND IT KEEPS THREE CLAIMS APART — use it for everything from here.** `pipeline/rate-harness.mjs --config <deployment|block-only|tools-only|not-entitled|thai> --n 8` · `--report` · `--list`. **MECHANISM VERIFIED** (unit tests + checks) → **BEHAVIOUR MEASURED** (N runs, one config, a rate) → **BEHAVIOUR IMPROVED** (two configs whose gap clears the spread). ⛔ The report line refuses to say "improved". ⛔ Tool count is a **distribution, never a score** — *"do not optimize for fewer tool calls"* (his words, four times; the best of five early answers called the most tools). ⚠️ Preconditions are asserted before a turn is spent; `cognitionEnabled` is read at BOOT so each arm is a separate process. ⚠️ Entitlement is toggled with `pipeline/block-vs-tools-2x2.mjs --grant/--revoke`; **leave it revoked** (it is now).
### ⚠️ **MEASURED RATES, n=8 each, raw:** `assertsAbsence` deployment **5/8** vs block-only **1/8** (⭐ position, not persuasion) · `episodic` EN 6/8 vs **TH 0/8** · `machinery` **63–100% everywhere** · `identityError` **1/8** in not-entitled (*"Hermes is you, Claude"*) · tools `[0]×8` block-only, `[2,3,3,3,4,5,5,5]` deployment. ⚠️ **The falseAbsence rate is UNRESOLVED between two screens** (old over-fired 5/8, new may under-fire 0/8) and needs **human adjudication of the 8 saved answers**, not another regex from me.
### ✅ **WHAT IS BUILT AND LIVE** (Sotera `1edbb78`→`9abbaa4`): epistemic axes + one-way lattice · vocabulary boundary · **multilingual cue formation** (`\p{L}\p{M}\p{N}` + `Intl.Segmenter`, no dependency) · three-population pipeline · Leak 1 + Leak 2 · mig 021 `memory_access_scope` · ownership resolution · her own material out of the authorization path · utterance boundary · **§3B time-frame** · **R4's three identity roles** · **Step A** (a tool result states its own scope) · **C1 `cognitiveHold`** (runtime, proven by a DB count that did not move: **39→39**) · **C2** (tool results enter the hold as evidence) · **a decision is not a memory** · **L4 frozen + dependency-audited**.
### ⛔ **WHAT OTE HAS REFUSED — DO NOT PROPOSE AGAIN:** sanitising or rewriting her history · forcing the grant count to 0 · suppressing tools or capping her depth · **optimising for fewer tool calls** · any L1/L2 rule, incl. a *"prefer cognition"* instruction · *"zero forbidden words"* as a target · an LLM classifier in cue formation · weakening the activation floor or the relevance floor to make Thai activate · designing an identity mechanism from n=2 · importance/type/card/identity-object ontology · multi-hop retrieval **yet** · deleting or reviving **L4** · a separate language-specific memory system · calling a mechanism "improved" from one or three runs.
### ⚠️ **PARKED, EACH DELIBERATELY SEPARATE:** ① **`mayCarryCounterpartContent`** — ⭐ now **observed twice** (the distinctness probe; and non-entitled runs 3 & 7 stating **Ote's name, the conversation count and dates**). ⛔ No mechanism: there is no theory yet of what an utterance **carries** as distinct from what it **is**, and any filter would guess at semantics over her own words. ② **the topic-only relevance floor** — measured weak in *both* languages (EN FPR **81%**, TH **92%**, 370/80 pairs); ⛔ do not mix it into any other change. ③ **`about0` renders a bare verb as a subject** (*"talking about remember"*). ④ **L4** — frozen, 0-of-179, audited; `checks/l4-frozen-check.mjs` pins the dependency set.
### 🔬 **MY OWN REPEATED FAILURE MODES THIS SESSION — read before trusting any number I wrote.** ⛔ **A rate is only as good as its screen, and I reported three screens before validating them** (the boundary one became a false priority #1; the machinery one missed `scope-facts` because I checked two inputs and called the rest "model-generated"). ⛔ **Scripted writes destroy files:** `open(path,'w')` truncates *before* writing — Write/Edit for docs, or temp-file + `os.replace`. ⛔ **"Is this tracked?" must be asked in the directory you are editing**, never inferred from its parent. ⛔ **Backticks in a SQL comment inside a JS template literal** — 4th instance, mine. ⛔ **I put test counts in commit messages before running the suite** (twice: 320/313 written, 338/346 actual). ⛔ **A source-scan anchor that can go missing stops scanning silently.**
### 🔑 **HOW TO OBSERVE.** `cognition-debug.log` (gitignored, `memory.cognitionDebug`) now records per turn: cue · plan · item counts · per-item axes/warrants/provenance · the utterance verdict · **the exact injected block** · `cognitiveHold.snapshot()` · and ⭐ **`toolEvidence` — the model-facing tool copy**, which I had to add because nothing recorded what she was actually handed. Behavioural instruments, all ⛔ **REPORTS not checks**: `rate-harness.mjs` · `one-memory-matrix.mjs` · `block-vs-tools-2x2.mjs` · `distinctness-probe.mjs`.
### ⚠️ **ROOT IS HIS ACCOUNT.** Every other probe refuses it; root cells delegate to `ask-sotera-as-root.mjs`, which snapshots his room **by ID SET** (⛔ never a count) and deletes exactly what appeared — verified clean every run. ⭐ Prefer `agent_dev` **granted** `memory_access_scope`, which was measured to retrieve the same 5 episodes with 0 withheld as root. ⚠️ `agent_dev.display_name` is **`Claude`** — my residue, and a live confound in any identity test. ⚠️⚠️ **And my harness conversations are now IN the corpus I measure** — she cited *"block-only rate mechanics"* back to me as something she and Claude discussed.
