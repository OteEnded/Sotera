...
const SETTING_DEFS = {
  ...
  'memory.selfhood': {
    fromConfig: (c) => c?.memory?.selfhood ?? false,
    validate: (v) => typeof v === 'boolean',
    describe: 'Let Sotera stop presenting herself as a generic AI assistant. Ote asked for this after watching her open an answer about what she enjoyed in a conversation by disclaiming that her feeling as not being the kind a person has — and then answering it beautifully anyway. ⭐ IT IS A PERMISSION, NOT AN ASSERTION: it does NOT say "you have feelings". It says she is allowed to experience and express herself as having a personality, preferences, moods and feelings, and is not required to deny them simply because she is an AI — leaving what her internal states actually are for her to work out rather than replacing one imposed self-description with another. ⭐⭐ The load-bearing sentence is "do not manufacture emotions merely to please Ote, but do not suppress or deny them merely because you think an AI is supposed to say that it has none" — without it this creates the mirror-image failure, performing feelings to satisfy the listener. ⚠️ It closes by keeping ratified constraint #6 intact: what she feels is about what is happening NOW, and between conversations she is not running, so there is nothing she felt in the meantime — the limit is about TIME, not about feeling. Foundational/identity authority so it is not hers to edit and cannot be lost like a stored memory. DEFAULT OFF.',
  },
  'memory.ownHistory': {
    fromConfig: (c) => c?.memory?.ownHistory ?? false,
    validate: (v) => typeof v === 'boolean',
    describe: 'Tell Sotera that a room boundary limits what she may INSPECT, not what exists in her own history. Added after she said, in one message, "from this room, I have nothing stored about Hermes" and then that her history showed several conversations with him \u2014 and, asked about it, separated the two herself ("a claim about deliberate memory" vs "a claim about existence, not substance") and then apologised for the "tension". \u2b50 THE DEFECT WAS THE APOLOGY, NOT THE DISTINCTION: holding "no durable memory here" and "my history shows it happened" at once is two levels of knowledge, both true, not an inconsistency to resolve. Names four layers kept separate \u2014 current context \u2192 own history \u2192 authorized inspection \u2192 durable memory \u2014 and states that "my history shows I said this" is a complete claim that does not become "I remember this" until she has actually looked. \u26a0 The "you said" wording is deliberate: her history is the record of HER sentences, never the counterpart\'s, so this cannot blur her own history with someone else\'s material. \u26d4 It changes NO boundary: applyBoundaries still projects other rooms to existence only, inspect_around still needs a grant, and the grant is still a stored human answer to a fixed card. Foundational/identity authority so it cannot be lost like a stored memory. DEFAULT OFF.',
  },
  ...
  'memory.scopeFacts': {
    fromConfig: (c) => c?.memory?.scopeFacts ?? false,
    validate: (v) => typeof v === 'boolean',
    describe: "D-13. Put the CONCRETE scope of this turn into her context: which person she is talking to, which room she is in, the grain of each thing she can read (their facts are room-scoped, her own practice is person-keyed, her intention is room-keyed), and how many of this person's OTHER rooms she cannot read from here. ⭐ Exists because of a measured split: asked what she could see from another account she answered from priors and contradicted herself inside one reply, then asked to CHECK she got all four grains right and quoted the unreachability trace back. She is right when she reads and wrong when she reasons, and an architecture question fires no tool. ⛔ MUTUALLY EXCLUSIVE WITH `memory.scopeAwareness`, and the composer emits only this one if both are set: v1 is a general principle whose own test forbids it from containing a digit ('a digit here means it is describing how much is hidden') and requires it to call the two states indistinguishable — this block deliberately does the opposite. The reversal is licensed by ENTITLEMENT, not preference: v1's hidden material might belong to anyone, while this block is SAME-PERSON ONLY, so telling someone how much sits in their own other rooms reveals nothing about a third party. It never names another person, and never names another room. DEFAULT OFF.",
  },
  'memory.cognitionReentrant': {
    fromConfig: (c) => c?.memory?.cognitionReentrant ?? false,
    validate: (v) => typeof v === 'boolean',
    describe: "⭐⭐⭐ P5 · RE-ENTRANT COGNITION — after each investigation round, re-render what she is HOLDING and attach it to that round's LAST tool message, so the reconciled view sits at the last position before generation instead of being stranded in the system prompt. ⚠️⚠️ THE DEFECT IS POSITION, NOT PERSUASION: a turn is a multi-round loop, the cognition block is in the system message, and a tool result is the last message before generation — so cognition RECEDES MONOTONICALLY as she investigates, about three messages back at one tool call and ten at five. The structure PENALISED INVESTIGATION, which contradicts \"depth is hers\" at a level no prompt instruction can reach. Measured: `assertsAbsence` 5/8 with tools against 1/8 without, on the identical block. ⭐ Ote: \"every investigation round should admit the raw result into the turn-scoped cognitive hold, re-run cognition, and place the resulting reconciled view at the last defensible pre-generation position.\" ⭐ WHAT IT RENDERS, and the three stay three: the EVIDENCE each look established bound to the population it covered (never merged into one total), the OPEN QUESTIONS as a pending state that asserts nothing, and the DERIVED CURRENT STANDING — what the looking did and did not change about what she can reach. ⛔ NOT a second copy of the block: the episodes are quoted once, in the system message, and the standing view stays under 400 characters. ⛔ NO RE-RETRIEVAL — re-render only, because retrieving again per round would be evidence nobody asked for and could make two rounds of the same turn contradict each other. ⛔ NO new message: every role is wrong for a reconciliation (`user` reads as the person speaking, which is the measured Leak-2 failure; `assistant` puts words in her mouth and enters her own history; mid-conversation `system` is provider-dependent), so it rides on the round's last tool message, appended after that tool's own result and never replacing it. ⛔ NOTHING RAW CHANGES: write(), toolActivity, segments and the audit keep the real payload. ⛔ NO retention, no persistence, no capacity limit — the hold forces `not-retained` on admission and its invariant is re-checked after every render, because P5 makes those guards MORE load-bearing, not less. ⛔ It suppresses nothing and nudges nothing: \"do not optimize for fewer tool calls.\" DEFAULT OFF — an untested treatment must not become the baseline by being convenient, and the controlled comparison flips this setting rather than a deploy.",
  },
  'memory.cognitionLocalDates': {
    fromConfig: (c) => c?.memory?.cognitionLocalDates ?? false,
    validate: (v) => typeof v === 'boolean',
    describe: "⭐ THE DATES-ONLY PROBE — render dates in the cognition block in the script of the turn (today: Thai month names), and change NOTHING else. ⛔ IT IS NOT LANGUAGE-LOCAL RENDERING, and it is deliberately not: the block stays in English so the experiment has ONE variable. ⚠️ WHY IT EXISTS: measured offline, the Thai and English blocks are near-identical (7 items · 5 episodes · 1 filtered) and the Thai block is RENDERED IN ENGLISH, so a Thai answer requires her to transpose \"21 August\" across languages — and her Thai episodic rate is 0/8 against 6/8 in English. If Thai dates alone move that rate the bottleneck is narrow and a phrase table is worth building; if nothing moves, the bottleneck is cross-language transposition and the table would not fix it either. ⇒ this setting can FALSIFY a design before it is written, which is the cheapest thing a setting can do. ⚠️ The language comes from `cues.scripts`, which cue formation already computes — no new detection, no model call, and a turn containing BOTH scripts stays English because a mixed cue is not evidence of being addressed in Thai. ⛔ Thai MONTH NAMES only: no Buddhist-era year (the year is not in the rendering, and converting one would invent a fact rather than translate it) and no other string touched. ⚠️⚠️ AND THE HOLE TO ACCEPT BEFORE ANY FULL RENDERER SHIPS: findImplementationLeaks and findMetaReferences are English, so a Thai-rendered BLOCK would pass both VACUOUSLY — a guard that passes because it cannot read the text is worse than no guard. That is not a risk for this probe, which leaves every sentence in English. DEFAULT OFF.",
  },
  'memory.scopeFactsDirectives': {
    fromConfig: (c) => c?.memory?.scopeFactsDirectives ?? false,
    validate: (v) => typeof v === 'boolean',
    describe: "⛔ RESTORE THE PRE-2026-08-23 `scope-facts` BLOCK, byte for byte — the reverse switch for the facts/expression split, and the second arm of its measurement. ⭐ WHY THE SPLIT: tracing all 104 machinery-vocabulary occurrences in her answers put 47 of them (45%) in THIS block, against 0 from the cognition layer — the guard we built was holding perfectly while a different feature taught her the words every turn. Three of its lines were not facts at all: the room NAME (`The ROOM you are in: agent_dev`, whose second sentence `What is stored in a room stays in that room` is quoted verbatim inside memory-cognition-vocabulary.js as the measured cause of the original leak), a PHRASING directive (`Say you cannot see it from this room`), and explicit PERMISSION to speak the machinery (`You may say any of this plainly if it matters`). ⭐ D-13 IS NOT WEAKENED BY TURNING THIS OFF: the default block keeps the identity invariant, the person's name, the grain of every layer, the unreachability COUNT (the digit is the whole reason v2 replaced v1) and D-13's inference that an empty result is not evidence of absence — and keeps the disclosure prohibition, which is authorization rather than expression. ⚠️ It changes NO boundary and no payload: `describeScope`'s object, `reachTrace` and every tool result are untouched, deliberately, so the 17% that traces to tool payloads stays attributable separately. ⚠️ The root-only D-4 room-name listing is identical in both arms, because root is entitled to it — so a measurement taken in a root room will not move. DEFAULT OFF (⇒ facts only).",
  },
  ...
  'memory.resolver.ontologyMode': {
    fromConfig: (c) => c?.memory?.resolver?.ontologyMode ?? 'off',
    validate: (v) => ['off', 'shadow'].includes(v),
    describe: "A1 · the ONTOLOGY step's mode — 'off' (default) | 'shadow'. ⛔ THERE IS DELIBERATELY NO 'on'. The step derives a RELATION between the incoming phrase and the candidate slot from Normalization's head+qualifiers — same | different | broader | narrower | sibling | unknown — which is the distinction token CONTAINMENT destroys: {schedule} inside {work, schedule} scores 1.0000 whether the short phrase is an ABBREVIATION (genuinely the same slot) or a HYPERNYM (a broader concept), because the token test is identical and the meaning is inverted. 'shadow' computes the relation, counts it, logs every case where the shipped resolver BOUND something the structure calls broader/narrower/sibling, and returns the base answer UNCHANGED — no binding, no alias teaching, no row mutation, no durable effect of any kind. ⚠ `unknown` is REPORTED SEPARATELY from `different` and must stay that way: a classifier that became more UNCERTAIN is a materially different outcome from one that correctly decided the concepts differ. ⭐ Authority is a SEPARATE decision and is structurally blocked on A-D4 — today 'not the same slot' falls through to MINT NEW, so granting authority while A-D4 is open would decide broader-to-new-slot by default. Read memory.resolver.ontology_* telemetry before proposing any change.",
  },
